...
def merge_data_flavour(test_run,analyser,skim,  label, flavours):
    ...
    for era in eras:
        ...
        out_file =local_dir+"/"+analyser+"_data_"+label+".root"
        if "NonPrompt" in label:
            out_file =local_dir+"/"+analyser+"_"+label+".root"
        ...
        print(' ' )
        for x in flavours:
            in_file =local_dir+"/"+analyser+"_data_"+x+".root "
            if "NonPrompt" in label:
                in_file =local_dir+"/"+analyser+"_"+x+".root "
...
def merge_cf(test_run,analyser,skim, flavour, flav_dir):
    ...
    for era in eras:
        ...
        out_file=local_dir+"/"+analyser+"_CF_"+flavour+".root"
        if os.path.exists(out_file):
            os.remove(out_file)
        string_hadd = ""
        _dir=flav_dir[n_era]
...
def merge_data(test_run,analyser,skim, flavour, flav_dir):
    ...
    for era in eras:
        ...
        out_file=local_dir+"/"+analyser+"_data_"+flavour+".root"
        if "NonPrompt" in flavour:
            out_file=local_dir+"/"+analyser+"_"+flavour+".root"
        if "CF" in flavour:
            out_file=local_dir+"/"+analyser+"_"+flavour+".root"
...
def merge_mc(test_run,analyser, skim):
    ...
    for era in eras:
        ...
        out_file=local_dir+"/"+analyser+"_SSPrompt.root"
        if os.path.exists(out_file):
            os.system('rm ' +out_file)
...
def merge_data_setup(_isTest,_analyser_name,_skim_name):
    ...
    if analyser_name == "HNL_Validation":
        ...
        merge_data(_isTest,_analyser_name,_skim_name,"EE",
                   [["","_SingleElectron"],["","_SingleElectron"],["","_SingleElectron"],["","_EGamma"]])
        merge_data(_isTest,_analyser_name,_skim_name,"MuMu",  
                   [["","_SingleMuon"],["","_SingleMuon"],["","_SingleMuon"],["","_SingleMuon"]])
