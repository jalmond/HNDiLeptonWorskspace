#ifndef HNLScanPlotter_h
#define HNLScanPlotter_h
...
class HNLScanPlotter{
public:
  //==== constructors
  HNLScanPlotter(TString macroname);
  ~HNLScanPlotter();
  ...
  void DrawScanCutFlowWithData(TString defaultName);
  void DrawComparisonPlots();
  void draw_hist();
  void draw_cutflow(TString defaultname="NULL");
  void draw_comphist();
  ...
  double  draw_canvas_fakescan(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> hist_signal, TLegend *legend, bool DrawData, TFile *outputf, TString TAG);
