	...
	for(const auto& channel: Channels) {
	  ...
	  for(const auto& id: IDs) {
	    ...
	    for(const auto& mass: masses) {
	      ...
	      if(h_fake&&_bin == "SR2" && input_hist.Contains("reco_ml")) h_fake->Scale(0.5);
              if(h_fake){
                h_fake->SetName("fake");
		fout->cd();
                h_fake->Write();
		...
		for(int ibin=0; ibin < h_fake->GetNbinsX()+1;ibin++){
	      ...
	      if(h_prompt&&_bin == "SR2" && input_hist.Contains("reco_ml")) h_prompt->Scale(0.5);
		...
		if(CheckHist(file_cf,input_hist)){
		  ...
		  if(h_cf&&_bin == "SR2" && input_hist.Contains("reco_ml")) h_cf->Scale(0.5);
		...
		if(mass_d > 250.){
	      ...
	      cout << h_sig->Integral();
	      if(h_sig&&_bin == "SR2" && input_hist.Contains("reco_ml")) _scale=_scale*0.5;
