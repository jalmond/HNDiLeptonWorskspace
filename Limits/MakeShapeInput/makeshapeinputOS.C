...
void makeshapeinputOS(int k=0,TString analysername="HNtypeI_JA"){
  ...
  vector<TString>  Years = {"2016","2017","2018"};//;,"2017","2018"};
  vector<TString> Bins={"SR4"};//,"SR2"};
  vector<TString> Flavours={"MuMu","EE"};
  vector<TString> Channels={"Schannel","Tchannel","Combinedchannel"};
  vector<TString> ChannelTags={"_DY","_VBFOnly","_VBF"};

  vector<TString> muIDs={"POGTightPFIsoVeryTight","HNTight2016","HNTightV1","POGTightPFIsoTight","POGTightPFIsoMedium","POGTightPFIsoLoose"};
	...
	for(const auto& channel: Channels) {
	  ...
	  for(const auto& id: IDs) {
	    ...
	    for(const auto& mass: masses) {
	      ...
	      if(h_fake&&_bin == "SR4" && input_hist.Contains("reco_ml")) h_fake->Scale(0.5);
		...
		for(int ibin=0; ibin < h_fake->GetNbinsX()+1;ibin++){
	      ...
	      TH1* h_prompt = GetHist(file_prompt, input_hist);
              ...
              if(h_prompt&&_bin == "SR4" && input_hist.Contains("reco_ml")) h_prompt->Scale(0.5);
		...
		if(CheckHist(file_cf,input_hist)){
		  ...
		  if(h_cf&&_bin == "SR4" && input_hist.Contains("reco_ml")) h_cf->Scale(0.5);
		...
		if(mass_d > 250.){
	      ...
	      if(h_sig&& _bin == "SR4" && input_hist.Contains("reco_ml")) _scale=_scale*0.5;
