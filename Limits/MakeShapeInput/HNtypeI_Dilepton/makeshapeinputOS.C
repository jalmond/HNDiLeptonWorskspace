...
void makeshapeinputOS(int k=0,TString analysername="HNtypeI_Dilepton"){
  ...
  vector<TString> Bins={"SR3","SR4"};//,"SR2"};
	...
	for(const auto& channel: Channels) {
	  ...
	  for(const auto& id: IDs) {
	    ...
	    for(const auto& mass: masses) {
	      ...
	      //if (_bin == "SR3" || _bin == "SR4") h_path_fake =  ENV_MERGEDFILE_PATH+ "/"+analysername+"/"+year+"/"+analysername+"_SkimTree_SSNonIso_Fake.root";
