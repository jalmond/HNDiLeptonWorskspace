...
check_limit_reader_path(input_dir,"SRComb12_EE_MM")
...
MissingLimit=[]
niter = NIteration([years, SRs, flavours,Channels,Vars])
outfiles = []
files_tocopy=[]
for _iter in range(0,niter):

       GetIter  = SumIteration(_iter, [years, SRs, flavours,Channels,Vars])
       year     = GetIter[0]
       _sr      = GetIter[1]
       flavour  = GetIter[2]
       channel       = GetIter[3]
       ...
       for _id in IDs:
              ...
              outfile = open(Outdir+ year+"/"+_allmassfilename+"/result"+channel_tag+"_"+_id+"_"+var+".txt","w")
              print "Creating file  " + Outdir+ year+"/"+_allmassfilename+"/result"+channel_tag+"_"+_id+"_"+var+".txt"
              for mass in _masses:
                     ...
                     #higgsCombinecard_2016_MuMu_SR1_N1000_VBF_HNTight2016_reco_ml2jj.txt_exp0.500.HybridNew.mH120.quant0.500.root

                     
                     _filename = Limitdir+  input_dir+"/higgsCombinecard_"+year+"_"+flavour + "_combined_"+_sr+"_N"+mass+channel_tag+"_"+_id+"_"+var+".txt_exp0.500.HybridNew.mH120.quant0.500.root"   

                     if not os.path.exists(_filename):
                            print "No file " + _filename
                            exit()
                     if os.path.exists(_filename):
                            os.system("root -l -q -b 'ReadLimitFromTree.C(\"/data6/Users/jalmond/Limits/CMSSW_10_2_13/src/HiggsAnalysis/CombinedLimit/data/2016_HNDiLepton/batch/"+input_dir+"/higgsCombinecard_"+year+"_"+flavour+"_combined_"+_sr+"_N"+mass+channel_tag+"_"+_id+"_"+var+".txt_exp0.500.HybridNew.mH120.quant0.500.root\",\"test\")'")
