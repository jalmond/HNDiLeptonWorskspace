#include "HNLStackPlotter.h"
#include <stdlib.h>

HNLStackPlotter::HNLStackPlotter(TString macroname){
...
void HNLStackPlotter::DrawStackPlots(){
  DrawDataAll(false);
  make_bkglist();
  draw_hist();
  return;
...
void HNLStackPlotter::DrawStackPlotsWithData(){
...
void HNLStackPlotter::DrawComparisonPlots(){
  if(!comp_default_set) return;
  if(FullHistNames.size() ==0) return;
  draw_comphist();
}

void HNLStackPlotter::SetupDefaultHist(TString sample, TString hn, TString histtag, TString legend, TString htype, TString hunit, vector<double> rb, double Xmin, double Xmax, double Ymax){
...
void HNLStackPlotter::SetupComparisonHist(TString sample, TString hist, TString histtag, TString legend){
...
void HNLStackPlotter::AddHist(TString hn, TString htype, TString hunit, vector<double> rb, double Xmin, double Xmax, double Ymax){
...
void HNLStackPlotter::BasicSetup(SetupHelper logy, SetupHelper ratio,  TString channel){
...
void HNLStackPlotter::SetupPlotter(TString era, TString Skim, TString Analyzer){
  SkimName=Skim;
  Era=era;
...
void HNLStackPlotter::SetupPlotter(){
...
TString HNLStackPlotter::GetTitleByType(TString htype ){
...
HNLStackPlotter::~HNLStackPlotter(){
...
void HNLStackPlotter::make_cutflow(TString Hist_For_CutFlow){
...
TH1D* HNLStackPlotter::MakeHist(TString filepath, TString fullhistname){
...
void HNLStackPlotter::draw_comphist(){
...
void HNLStackPlotter::draw_hist(){
...
void HNLStackPlotter::make_bkglist(){
...
void HNLStackPlotter::SetRebins(TString filepath){

  cout << "[HNLStackPlotter::SetRebins] Get rebins from " << filepath << endl;
...
void HNLStackPlotter::SetYAxis(TString filepath){

  cout << "[HNLStackPlotter::SetYAxis] Get Yaxis from " << filepath << endl;
...
void HNLStackPlotter::SetXAxis(TString filepath){

  cout << "[HNLStackPlotter::SetXAxis] Get Xaxis from " << filepath << endl;
...
vector<double> HNLStackPlotter::GetRebinVariableBins(){
...
void HNLStackPlotter::MakeRebins(){
...
void HNLStackPlotter::MakeYAxis(){
...
void HNLStackPlotter::MakeXAxis(){
...
TString HNLStackPlotter::find_MCsector(){
...
void HNLStackPlotter::clear_legend_info(){
...
double HNLStackPlotter::coupling_constant(int mass){
...
void HNLStackPlotter::fill_legend(TLegend* lg, TH1D* hist){
...
void HNLStackPlotter::draw_legend(TLegend* lg, bool DrawData){
  // Example :
  //                      0    1    2    3
...
void HNLStackPlotter::draw_comp_canvas(TH1D *hist_def, TH1D *hist_comp, TLegend *legend ,TFile *outputf){
...
void HNLStackPlotter::draw_canvas(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> hist_signal, TLegend *legend, bool DrawData, TFile *outputf){
...
void HNLStackPlotter::SetXaxisRange(TH1D* hist){
...
void HNLStackPlotter::SetXaxisRange(THStack* mc_stack){
...
void HNLStackPlotter::SetXaxisRangeBoth(THStack* mc_stack, TH1D* hist){
...
TH1D* HNLStackPlotter::MakeOverflowBin(TH1D* hist){
...
TString HNLStackPlotter::DoubleToString(double dx){

  //cout << "[HNLStackPlotter::DoubleToString] var = " << HistNames[i_var] << endl;
  //cout << "[HNLStackPlotter::DoubleToString] unit = " << units[i_var] << endl;
  //cout << "[HNLStackPlotter::DoubleToString] dx = " << dx << endl;
...
TString HNLStackPlotter::legend_coupling_label(int mass){
...
void HNLStackPlotter::mkdir(TString path){
...
double HNLStackPlotter::GetHistValue(TH1D * ht, TString hn){
...
double HNLStackPlotter::GetHistError(TH1D * ht, TString hn){
...
TString HNLStackPlotter::FixLatex(TString origSt){
  TString fixSt=origSt;
  if(fixSt.Contains("P_T")) fixSt.ReplaceAll("P_T","$\\mathrm{P}_{T}$");
  return fixSt;
}
void HNLStackPlotter::MakeTexFile(map< TString, TH1D * > hs, TString Hist_For_CutFlow){
...
void HNLStackPlotter::Summary(){
  
  cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
  cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
  cout << " "<< endl;
  cout << "HNLStackPlotter Ran StackPlots : With Data. Results sent to : " << endl;
...
TString HNLStackPlotter::GetStringChannelRegion(int A, TString B){
...
bool HNLStackPlotter::ZeroDataCheckCut(double xlow, double xhigh){
...
void HNLStackPlotter::SetupSampleInfo(){
...
void HNLStackPlotter::LeptonChannel(TString ch){
...
void HNLStackPlotter::UseLogyAll(double v){
  UseLogy.clear();
  for(int i=0; i < HistPath.size() ; i++) UseLogy.push_back(v);
}

void HNLStackPlotter::ApplyMCNormSFAll(bool v){
  ApplyMCNormSF.clear();
  for(int i=0; i < HistPath.size()  ; i++)ApplyMCNormSF.push_back(v);
}
void HNLStackPlotter::DrawRatioAll(bool v){

  drawratio.clear();
  for(int i=0; i < HistPath.size()  ; i++) drawratio.push_back(v);
}

void HNLStackPlotter::DrawDataAll(bool v){

  drawdata.clear();
  for(int i=0; i < HistPath.size()  ; i++) drawdata.push_back(v);
}

vector<double> HNLStackPlotter::GetRebinZeroBackground(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> &hist_signal){
