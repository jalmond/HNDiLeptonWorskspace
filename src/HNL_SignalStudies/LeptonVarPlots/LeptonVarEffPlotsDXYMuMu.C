  ...
  for (auto year : eras){
	...
	for (auto var : vars){
	  ...
	  for(int ibin = 0; ibin < 4;  ibin++){
	    ...
	    for( int j1=0; j1 < 46; j1++){
	      double DXY_1 = double(0.005+0.001*double(j1));
	      std::string DXY1= std::to_string(DXY_1);

	      
	      TString sr_hist=  "OptMuMu/Sig"+var+"_"+TString(DXY1);
	      TString sr_Fakehist=  "OptMuMu/Fake"+var+"_"+TString(DXY1);
	      TString sr_Convhist=  "OptMuMu/Conv"+var+"_"+TString(DXY1);
