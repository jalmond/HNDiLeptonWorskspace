...
void LeptonVarEffPlotsIP3D(){
  ...
  for (auto year : eras){
	...
	vector<TString> vars = {"B/LepPt_IP3D", "EC/LepPt_IP3D"};
	...
	for (auto var : vars){
	  ...
	  for(int ibin = 0; ibin < 4;  ibin++){
	    ...
	    for(unsigned int j1=0; j1 < n; j1++){
	      ...
	      TString sr_hist=  "OptEE/Sig"+var+"_"+TString(IP1);
	      TString sr_Fakehist=  "OptEE/Fake"+var+"_"+TString(IP1);
	      TString sr_ExtConvhist=  "OptEE/ExtConv"+var+"_"+TString(IP1);
	      TString sr_IntConvhist=  "OptEE/IntConv"+var+"_"+TString(IP1);
	      TString sr_CFhist=  "OptEE/CF"+var+"_"+TString(IP1);
	      ...
	      if( (100*(binned_int / nom_int) ) > 95. && sr_hist_95=="") sr_hist_95=TString(IP1);
	      if( (100*(binned_int / nom_int) ) > 99. && sr_hist_99=="") sr_hist_99=TString(IP1);
	      if( (100*(binned_int / nom_int) ) > 99.5 && sr_hist_995=="") sr_hist_995=TString(IP1);
	    }
