...
void LeptonVarEffPlotsDXY(){
  ...
  for (auto year : eras){
	...
	vector<TString> vars = {"B/LepPt_DXY", "EC/LepPt_DXY"};
	...
	for (auto var : vars){
	  ...
	  for(int ibin = 0; ibin < 4;  ibin++){
	    ...
	    for(unsigned int j1=0; j1 < n; j1++){
	      ...
	      TString sr_hist=  "OptEE/Sig"+var+"_"+TString(DXY1);
	      TString sr_Fakehist=  "OptEE/Fake"+var+"_"+TString(DXY1);
	      TString sr_ExtConvhist=  "OptEE/ExtConv"+var+"_"+TString(DXY1);
	      TString sr_IntConvhist=  "OptEE/IntConv"+var+"_"+TString(DXY1);
	      TString sr_CFhist=  "OptEE/CF"+var+"_"+TString(DXY1);
	      ...
	      if( (100*(binned_int / nom_int) ) > 95. && sr_hist_95=="") sr_hist_95=TString(DXY1);
	      if( (100*(binned_int / nom_int) ) > 99. && sr_hist_99=="") sr_hist_99=TString(DXY1);
	      if( (100*(binned_int / nom_int) ) > 99.5 && sr_hist_995=="") sr_hist_995=TString(DXY1);
	    }
