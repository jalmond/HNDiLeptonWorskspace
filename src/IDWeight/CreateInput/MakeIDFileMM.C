...
void MakeIDFileMM(TString year="2016",TString dataset="Muon"){
  ...
  TString outfile = "IDRate13TeV_mu_"+year+".root";
  ...
  for(unsigned int i=0; i < fakes40.size(); i++){
    ...
    TString denom = "Z_MuMu_/Z_MuMu__lep_pt_eta_HNtypeI_JA_MuMu_"+fakes40[i];
    
    //    return;
    TH2* h_nV_mc   = (TH2*)fmc->Get(denom.Data());
    TH2* h_nV_data = (TH2*)fdata->Get(denom.Data());

    CheckHist(h_nV_mc);
    CheckHist(h_nV_data);
    TString name = fakes40[i]+"NVertexReweights" ;

    
    TH2* _h_nV_mc = (TH2*)h_nV_mc->Clone((name+"MC").Data());
    TH2* _h_nV_data = (TH2*)h_nV_data->Clone((name +"DATA").Data());
