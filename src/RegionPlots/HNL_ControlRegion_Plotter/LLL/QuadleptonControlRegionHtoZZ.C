...
void QuadleptonControlRegionHtoZZ(){

  //  vector<TString> eras =  {"2016preVFP", "2016postVFP", "2017","2018"};
  vector<TString> eras =  {"2017"};
  vector<TString> channels = {"EMuLL"};
  
  for (auto year : eras){
    for (auto channel : channels){
      
      HNLPlotter Plotter("HNL_HtoZZ");
      //// change def
      ...
      //// Setup plotter
      Plotter.SetupPlotter(year,"SkimTree_HNMultiLepBDT", "HNL_HtoZZ");
      ///Plotter.infilepath = .... if inut file is not in Mergeddir

      //Plotter.samples_to_use = {"ttbar_ll","DY","VV_incl"};

      Plotter.samples_to_use = {"ZZ","HtoZZ","Other"};
      //Plotter.ScaleSample("NonPrompt",0.2);
      ///Plotter.ScaleSample("WZ_pythia",0.95);
      Plotter.HistPath= {"HNL_ZZLoose_FourLepton_CR/RegionPlots_"+channel+"/HNTightV2",                                                                                                                     
			 "HNL_ZZLoose_FourLepton_CR/RegionPlots_"+channel+"/HNL_ULID_2017"};

      Plotter.RegionType = {"ZZ CR","ZZ CR"};
      ...
      Plotter.AddHist("Lep_4_pt" ,"LPT4" , "GeV", {1.}, 10, 200);
      Plotter.AddHist("Lep_1_eta" ,"Eta" , "", {4.}, -3, 3);
      Plotter.AddHist("Lep_2_eta" ,"Eta" , "", {4.}, -3, 3);
      Plotter.AddHist("Lep_3_eta" ,"Eta" , "", {4.}, -3, 3);
      Plotter.AddHist("Lep_4_eta" ,"Eta" , "", {4.}, -3, 3);

      //Plotter.AddHist("Lep_3_pt" ,"LPT3" , "GeV",  {10.,15., 20., 40., 50., 200}, 0, 200);    
