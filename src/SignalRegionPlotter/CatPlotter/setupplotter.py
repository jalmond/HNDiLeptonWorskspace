...
def  MakeConfFile(inputdir,jobname,datastream,analyzer,tag,tag2,filedate,cut,leglist, plottag, isblind,year,_id):
    ...
    file_conf.write("id          " + _id+"\n")
    file_conf.write("prefix          " + analyzer + tag+"\n")
    file_conf.write("postfix           " + tag2+" .root\n")
    file_conf.write("# NAME_THE_OUTPUT_DIR\n")
    file_conf.write("outputdir  HN13TeV_"+analyzer+"_"+str(year)+"_"+filedate+"_"+cut+"\n")
    file_conf.write("histdir    SK"+str(year)+"_"+plottag+"\n")
...
parser.add_option("-j", "--j",  dest="j", default="",help="tag")
    ...
    for line in input_config:
        if (" " + i) in line:
            ...
            if(len(sline) ) > 0:
                tag="_"
                tag2=""
                if  skim == "SkimTree_SSNonIso":
                    tag="_SkimTree_SSNonIso_"
                    tag2=""

                prefix= analyzer + tag
                postfix= tag2+".root"

                if os.path.exists(inputdir+"/"+year+"/"+prefix+sline[0]+postfix):
                    sample_exists=True
                    list_of_samples.append(sline[0])
                else:
                    print "Missing " + inputdir+"/"+year +"/"+prefix+sline[0]+postfix
