...
int MakeCutFlow_Plots(string configfile){
  ...
  output_path = string(getenv("HOME") )+"/SKAnalyzer/"+ path ;
  output_index_path = string("https://" + string(getenv("USER") )+".web.cern.ch/" + string(getenv("USER") )+"/SNU/SKAnalyzer/")+ path + "/histograms/" + histdir  + "/indexCMS.html";
...
void MakeCutFlow(string type){
  ...
  while(!cut_name_file.eof()) {
    ...
    cout << "$$$$$$$$$ " << hist << " $$$$$$$$$$$$$$$$$$$$" << endl;
    cuts.push_back((cutname+ "/"+cutname + "_"+hist +"_HNtypeI_JA_" + id).c_str());    
  }
  ...
  for(vector<string>::iterator it = cuts.begin(); it!=cuts.end(); it++, i_cut++){
    ...
    string mv_command = "mv " + cut_label.at(i_cut) +".pdf /home/" + string(getenv("USER") )+"/SKAnalyzer/" + path +"/histograms/"+ histdir ;
...
void PrintCanvas(TCanvas* c1, string folder, string plot_description, string title){
  ...
  std::string tpdf = string(getenv("HOME") )+"/SKAnalyzer/"+ path +  "/histograms/"+folder+"/"+title;
...
legorder.push_back("prompt");
legorder.push_back("chargeflip");
legorder.push_back("nonprompt");
...
if(sample.Contains("SSPrompt")){    list.push_back(make_pair("SSPrompt",0.20));
}if(sample.Contains("CF")){    list.push_back(make_pair("CF",0.4));
}if(sample.Contains("FakeEE")){    list.push_back(make_pair("FakeEE",0.3));
}  
...
THStack* MakeStack(vector<pair<pair<vector<pair<TString,float> >, int >, TString > > sample, TString type, string name, float xmin, float xmax,map<TString, TH1*>& legmap , int rebin, bool include_syst_err){
  ...
  for(vector<pair<pair<vector<pair<TString,float> >, int >, TString > >::iterator it = sample.begin() ; it!= sample.end(); it++){
    ...
    cout << it->second << endl;
    vector<pair<TString,float> > p_ = it->first.first;
    for(unsigned int i = 0 ; i < p_.size(); i++){
      cout << p_[i].first << " " << p_[i].second << endl;
    }
  }
...
vector<pair<TString,float> >  SSPrompt = InitSample(" SSPrompt"); 
vector<pair<TString,float> >  CF = InitSample(" CF"); 
vector<pair<TString,float> >  FakeEE = InitSample(" FakeEE"); 


  for( unsigned int i = 0; i < listofsamples.size(); i++){
   if(listofsamples.at(i) =="SSPrompt")samples.push_back(make_pair(make_pair(SSPrompt,kSpring-1),"prompt"));
   if(listofsamples.at(i) =="CF")samples.push_back(make_pair(make_pair(CF,kCyan),"chargeflip"));
   if(listofsamples.at(i) =="FakeEE")samples.push_back(make_pair(make_pair(FakeEE,870),"nonprompt"));
...
bool drawsig=true;
  ...
  TString loc_=TString(mcloc);
  if(loc_.Contains("2016")) CMS_lumi( canvas, 4, 11 );
  if(loc_.Contains("2017")) CMS_lumi( canvas, 5, 11 );
  if(loc_.Contains("2018")) CMS_lumi( canvas, 6, 11 );
  //return canvas;  
  canvas->Update();
  canvas->RedrawAxis();
