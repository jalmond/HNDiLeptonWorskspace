...
if(sample.Contains("SSPrompt")){    list.push_back(make_pair("SSPrompt",0.20));
 ...
 if(sample.Contains("FakeMuMu")){    list.push_back(make_pair("FakeMuMu",0.3));
 }  
...
vector<pair<TString,float> >  FakeMuMu = InitSample(" FakeMuMu"); 
  ...
  for( unsigned int i = 0; i < listofsamples.size(); i++){
   ...
   if(listofsamples.at(i) =="FakeMuMu")samples.push_back(make_pair(make_pair(FakeMuMu,870),"nonprompt"));
