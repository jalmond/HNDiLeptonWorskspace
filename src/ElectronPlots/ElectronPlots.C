...
void ElectronPlots(TString year = "2016"){
  ...
  for (auto var: vars){
    ...
    TH1* hn_nonfake        = GetHistFull(file_mc,"NonFakeel/"+var, kRed, 3.);
    TH1* hn_nonfake_cf     = GetHistFull(file_mc,"NonFake_CFel/"+var, kGreen, 3.);
    TH1* hn_nonfake_conv   = GetHistFull(file_mc,"NonFake_Convel/"+var, kBlue, 3.);
    TH1* hn_nonfake_cfconv = GetHistFull(file_mc,"NonFake_Conv_CFel/"+var, kCyan, 3.);
    
    TH1* hn_fake           = GetHistFull(file_mc,"Fakeel/"+var, kRed, 3.,7);
    TH1* hn_fake_conv      = GetHistFull(file_mc,"Fake_Convel/"+var, kBlue, 3.,7);
