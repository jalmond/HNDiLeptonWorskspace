...
void MakeIDEfficiencyPlots(){
  ...
  for (auto era : eras){
	...
	TString tag = dataset+"_"+era+"_"+EtaBin;
	...
	TString denom = "MuMu/Pt_"+EtaBin+"_HNL_ULID_"+year+"_denom";
	TString num   = "MuMu/Pt_"+EtaBin+"_HNL_ULID_"+year+"_num";

	TString denomNP="MuMu/Pt_"+EtaBin+"_HNL_ULID_"+year+"_NonPrompt_denom";
	TString numNP="MuMu/Pt_"+EtaBin+"_HNL_ULID_"+year+"_NonPrompt_num";
	    
	if(dataset == "Electron"){
	  denom = "EE/Pt_"+EtaBin+"_HNL_ULID_"+year+"_denom";
	  num   = "EE/Pt_"+EtaBin+"_HNL_ULID_"+year+"_num";
	  denomNP="EE/Pt_"+EtaBin+"_HNL_ULID_"+year+"_NonPrompt_denom";
	  numNP="EE/Pt_"+EtaBin+"_HNL_ULID_"+year+"_NonPrompt_num";
	}
	
	TH1D* Hist_MC_Num        = GetHist(fmc,num);
	TH1D* Hist_MC_Den        = GetHist(fmc,denom);
      
	TH1D* Hist_FakeMC_Num        = GetHist(fmcAll,numNP);
        TH1D* Hist_FakeMC_Den        = GetHist(fmcAll,denomNP);
	...
	TString outpath = output + "/IDSF_"+era+"_"+EtaBin+"_"+dataset;
	...
	SaveAndCopyLXPLUS(c1,outpath,analysername,"IDSF_Muon");
      }
    }
