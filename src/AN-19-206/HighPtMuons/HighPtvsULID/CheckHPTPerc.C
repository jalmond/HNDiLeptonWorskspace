...
void DrawConfig(TString DateTag ,double mass, TString smass){
  ...
  TH1D *hist_HPT = Plotter.ConstructHist(path+"HNL_HighPtMuon_Studies_SkimTree_HNMultiLepBDT_DYTypeI_DF_M"+smass+"_private.root","SS/Muon1_Pt_Muon_HighPt_v3");
  TH1D *hist_ULID = Plotter.ConstructHist(path+"HNL_HighPtMuon_Studies_SkimTree_HNMultiLepBDT_DYTypeI_DF_M"+smass+"_private.root","SS/Muon1_Pt_Muon_ULID_v3");
  ...
  channelname.DrawLatex(0.2, 0.88,"CCDY Mass "+smass);
  ...
  TString HNAME = DateTag+"_SignalComp_Pt200_DY_"+ID +"_"+smass ;
