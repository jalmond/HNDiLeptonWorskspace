...
for Era in Eras:
    ...
    file_path=InputDir+ Era+"/HNL_SignalRegion_Plotter_SkimTree_HNMultiLepBDT_Bkg.root"
    file_path_os=InputDir+Era+"/HNL_SignalRegion_Plotter_SkimTree_HNMultiLepBDT_FullBkg.root"
        ...
        if histogram and isinstance(histogram, ROOT.TH1):
            ...
            for bin_idx in range(1, histogram.GetNbinsX() + 1):  # Bins are 1-indexed in ROOT
                ...
                if bin_label:  # If the bin has a label
                    print(f"{Era} Bin {bin_idx}: Label = {bin_label}, Content = {bin_content}")
                else:  # If the bin does not have a label
                    print(f"{Era} Bin {bin_idx}: Center = {bin_center}, Content = {bin_content}")
        else:
