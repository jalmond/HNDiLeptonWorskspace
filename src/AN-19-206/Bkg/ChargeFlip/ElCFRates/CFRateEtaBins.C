...
void CFRateEtaBinsIndiv(TString etabin){
  ...
  for (auto Era : Eras){
    ...
    if(etabin == "EtaRegion5" ){
      ...
      TH1D *hist_eta3_Numerator     = Plotter.ConstructHist(path, ID+"/CFRate/PBSExtrap_Central/EtaRegion7_Num");
      TH1D *hist_eta3_Denominator   = Plotter.ConstructHist(path, ID+"/CFRate/PBSExtrap_Central/EtaRegion7_Denom");
      hist_eta1_Numerator->Add(hist_eta3_Numerator);
      hist_eta1_Denominator->Add(hist_eta3_Denominator);

    }
    
    cout << hist_eta1_Numerator->Integral() << " " << hist_eta1_Denominator->Integral() << endl;

    hist_eta1_Numerator->GetXaxis()->SetTitle("Electron 1/p_{T} (GeV)^{-1}");
    
    hist_eta1_Numerator->GetYaxis()->SetTitle("CF_{Rates}");
    if(!Era.Contains("2016")){
      if(etabin == "EtaRegion1") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.005);
      if(etabin == "EtaRegion2") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.01);
      if(etabin == "EtaRegion3") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.025);
      if(etabin == "EtaRegion5") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.03);
      if(etabin == "EtaRegion7") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.03);
    }
    else{
      if(etabin == "EtaRegion1") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.1);
      if(etabin == "EtaRegion2") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.1);
      if(etabin == "EtaRegion3") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.00001,0.5);
      if(etabin == "EtaRegion5") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.00001,0.5);
      if(etabin == "EtaRegion7") hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.00001,0.5);
    }
    hist_eta1_Numerator->GetXaxis()->SetRangeUser(0.00,0.07);
    if(etabin == "EtaRegion7")    {
      hist_eta1_Numerator->GetXaxis()->SetRangeUser(0.002,0.07);
    ...
    if(etabin == "EtaRegion5") Label += "   1.556 < |#eta| < 2.5";
