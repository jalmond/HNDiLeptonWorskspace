...
void PlotClosureCF_LepPt(){
  ...
  for(auto era : Eras) {
    ...
    RunClosure("Bin3","InvPtEta3", Plotter, "HNL_ULID", era, "Oct8","PBS","HNL_ChargeFlip_Closure_Z", "Separate_BinnedShift", "Closure_ElectronCF_Z" ,  {20,25,30,40,50,60,70,80,90,100,150,200,300,400,500});
    RunClosure("Bin3","InvPtEta3", Plotter, "passHEEPID_v3", era, "Oct8","PBS","HNL_ChargeFlip_Closure_Z", "Separate_BinnedShift", "Closure_ElectronCF_Z" ,  {20,25,30,40,50,60,70,80,90,100,150,200,300,400,500});
...
void RunClosure(TString sbin, TString binning,HNLPlotter Plotter,TString ID, TString Era, TString  DateTag, TString Method, TString HistString, TString Indir,TString LabelForOutPut,vector<double> vrebin){
  ...
  TH1D *hist_Prompt              = Plotter.ConstructHist(path,ID+"/"+Indir+"_"+sbin+"/CFRate_"+binning+"_"+HistLab+"_Central_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  TH1D *hist_PromptUp              = Plotter.ConstructHist(path,ID+"/"+Indir+"Up_"+sbin+"/CFRate_"+binning+"_"+HistLab+"_PSigma_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  TH1D *hist_PromptDown              = Plotter.ConstructHist(path,ID+"/"+Indir+"Down_"+sbin+"/CFRate_"+binning+"_"+HistLab+"_MSigma_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  TH1D *hist_PromptStatUp              = Plotter.ConstructHist(path,ID+"/Separate_BinnedStatUp_"+sbin+"/CFRate_"+binning+"_"+HistLab+"_Central_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  TH1D *hist_PromptStatDown              = Plotter.ConstructHist(path,ID+"/Separate_BinnedStatDown_"+sbin+"/CFRate_"+binning+"_"+HistLab+"_Central_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  ...
  cout << hist_mass_CF << " " << hist_Prompt << endl;
  cout << hist_PromptUp << " " << hist_PromptDown << "  " << hist_PromptStatUp  << " " << hist_PromptStatDown << endl;
 

  cout << ID+"/Separate_BinnedStatUp_"+sbin+"/CFRate_"+binning+"_"+HistLab+"_SystUp_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt" << endl;
  ...
  double statup_int = hist_PromptStatUp->Integral() ;
  double statdown_int = hist_PromptStatDown->Integral() ;
  err = (hist_mass_CF->Integral() / hist_Prompt->Integral()  ) - ( hist_mass_CF->Integral() / (statup_int ));
  cout << "SF = " << hist_mass_CF->Integral() / hist_Prompt->Integral()  << " + / - " <<  err  << endl;
  SaveHistogram( Plotter,{hist_mass_CF,hist_Prompt,hist_PromptUp,hist_PromptDown,hist_PromptStatUp,hist_PromptStatDown}, {"SS Observed", "OS*R_{CF} Predicted"}, ID+"_CF_Closure_"+HistString+"_"+Method+"_"+Era, LabelForOutPut+"_"+sbin, {Method});
