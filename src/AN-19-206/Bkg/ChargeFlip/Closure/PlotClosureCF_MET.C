...
void RunClosure(HNLPlotter Plotter,TString ID, TString Era, TString DateTag, TString Dir, vector<TString> HName,TString LabelForOutPut, vector<double> rbins);
...
void PlotClosureCF_MET(){
  ...
  for(auto era : Plotter.Eras()) {
    ...
    for(auto st : ShiftTypes){
      ...
      RunClosure(Plotter, "HNL_TC1_ULID_"+year, era, "May5",st,{"CFRate_InvPtEta3_PBSExtrap_HNL_TC1_ULID_"+year,"MET","OS"},"HNL_ChargeFlip_Closure_Z_MET_"+st, {2});
...
void RunClosure(HNLPlotter Plotter,TString ID, TString Era, TString  DateTag,TString Method, vector<TString> vHistString, TString LabelForOutPut,vector<double> vrebin){
  
  Plotter.SetupPlotter(Era,"","HNL_Lepton_ChargeFlip");
  
  TString HistString;
  TString sHistString;
  for(auto i : vHistString) HistString=HistString+ "/"+i ; 
  for(auto i : vHistString) sHistString=sHistString+ "_"+i ; 
  ...
  cout << ID+"/"+Method+"/"+HistString << endl;
  ...
  TString HistStringNS = ID+"/"+Method+HistString + "_EnergyShift";
  TH1D *hist_Prompt_NoShift      = Plotter.ConstructHist(path,HistStringNS, vrebin);
  ...
  SaveHistogram( Plotter,{hist_mass_CF,hist_Prompt,hist_Prompt_NoShift}, {"SS Observed", "OS*R_{CF} Predicted","OS*R_{CF} Predicted [no shift]"}, ID+"_CF_Closure_"+sHistString+"_"+Method+"_"+Era, LabelForOutPut, {Method,Chi2Label});
