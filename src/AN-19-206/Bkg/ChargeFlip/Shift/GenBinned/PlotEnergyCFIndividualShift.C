...
void PlotEnergyCFIndividualShift(){
  ...
  for(auto era : Eras){
    ...
    for(auto etabin :  etabins){
      Plotter.XaxisMin = -0.5;  Plotter.XaxisMax = 0.09999;
      ...
      for(auto x :HistStrings)	MinChiResults.push_back(era + "_"+CF_EnergyShift_Proccessor(Plotter, "POGTight", era,TString(etabin+"_Pt_"+x), "AN_2019_206_HNL_ChargeFlip_GENBINNED_EnergyShift_"+TString(etabin+x), {1}));
      for(auto x :HistStrings)  MinChiResults.push_back(era + "_"+CF_EnergyShift_Proccessor(Plotter, "HNL_ULID", era,TString(etabin+"_Pt_"+x), "AN_2019_206_HNL_ChargeFlip_GENBINNED_EnergyShift_"+TString(etabin+x), {1}));
    }
...
TString CF_EnergyShift_Proccessor(HNLPlotter Plotter,TString ID, TString Era, TString HistString, TString LabelForOutPut,vector<double> vrebin){
  ...
  cout << "CF_EnergyShift_Proccessor:: " << ID+"/GenEnergyShift/"+HistString+"_CF" << endl;
  TH1D *hist_CF             = Plotter.ConstructHist(pathCF,ID+"/GenEnergyShift/"+HistString+"_CF", vrebin);
  hist_CF->Scale(1./hist_CF->Integral());

  TH1D *hist_Prompt         = Plotter.ConstructHist(pathP,ID+"/GenEnergyShift/"+HistString+"_PromptShifted_1", vrebin);
  ...
  for(auto ShiftVal : ShiftVals){
    ...
    cout << "CF_EnergyShift_Proccessor:: " << ID+"/GenEnergyShift/"+HistString+"_PromptShifted_"+ShiftVal << endl;
    TH1D *hist_PromptScaled   = Plotter.ConstructHist(pathP,ID+"/GenEnergyShift/"+HistString+"_PromptShifted_"+ShiftVal, vrebin);
