...
void DrawConfig(TString DateTag,TString Flavour, TString era, TString filepath,  TString ID){
  ...
  for (auto year : eras){
    ...
    vector<TString> GetMassListWW  = {"1000",  "1100", "1300", "1500", "2000",  "2500", "500"};

   
    TH1D * h_eff_DY = new TH1D ("","",GetMassListDY.size(),0,GetMassListDY.size());
    h_eff_DY->GetXaxis()->SetTitle("m_{N} (GeV)");
    h_eff_DY->GetYaxis()->SetRangeUser(0.7,1);
    h_eff_DY->GetYaxis()->SetTitle("m_{N} Selection efficiency for HEM cleaning");

    TH1D * h_eff_DY_SR1 = new TH1D ("","",GetMassListDY.size(),0,GetMassListDY.size());
    TH1D * h_eff_DY_SR2 = new TH1D ("","",GetMassListDY.size(),0,GetMassListDY.size());
    ...
    for(unsigned int i = 0 ; i < GetMassListDY.size(); i++){
      ...
      TH2D *hist_Inc        = Plotter.Construct2DHist(path_mass,"HNL_ULID/EE_Jet/Inclusive_El_HEM_EtaPhi");
      TH2D *hist_Fail       = Plotter.Construct2DHist(path_mass,"HNL_ULID/EE_Jet/HEM_Region_Fail_El_HEM_EtaPhi");
      ...
      h_eff_DY->SetBinContent(i+1,  EffHEM);
    }
    ...
    lg->SetBorderSize(0);
    lg->SetTextSize(0.03);
    lg->AddEntry(h_eff_DY,"Electron HEM","l");
    lg->Draw();

    TLatex channelname;
    channelname.SetNDC();
    channelname.SetTextSize(0.03);
    channelname.DrawLatex(0.2, 0.8,"CCDY");
