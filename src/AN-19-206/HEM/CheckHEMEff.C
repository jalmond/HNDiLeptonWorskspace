...
void DrawConfig(TString DateTag,TString Flavour, TString era, TString filepath,  TString ID){
  ...
  for (auto year : eras){
    ...
    for(unsigned int i = 0 ; i < GetMassListDY.size(); i++){
      TString path1_mass = path1 + "HNL_SignalRegion_Plotter_SkimTree_HNMultiLepBDT_DYTypeI_DF_M"+GetMassListDY[i] + "_private.root";
      TString path2_mass = path2 + "HNL_SignalRegion_Plotter_SkimTree_HNMultiLepBDT_DYTypeI_DF_M"+GetMassListDY[i] + "_private.root";
      ...
      vector<double> vrebin = {0.,5,10,15.,20};
      if(i > 10) vrebin = {0.,5,10,20};
      ...
      TString HNAME = DateTag+"_SignalEff_DY_"+GetMassListDY[i]+"_"+ID ;
