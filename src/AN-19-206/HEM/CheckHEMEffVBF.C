...
void DrawConfig(TString DateTag,TString Flavour, TString era, TString filepath,  TString ID){
  ...
  for (auto year : eras){
    ...
    for(unsigned int i = 0 ; i < GetMassListVBF.size(); i++){
      TString path1_mass = path1 + "HNL_SignalRegion_Plotter_SkimTree_HNMultiLepBDT_VBFTypeI_DF_M"+GetMassListVBF[i] + "_private.root";
      TString path2_mass = path2 + "HNL_SignalRegion_Plotter_SkimTree_HNMultiLepBDT_VBFTypeI_DF_M"+GetMassListVBF[i] + "_private.root";
      ...
      TCanvas* c1 = new TCanvas(Flavour+"_VBFEff", "", 1200, 800);
      c1->Draw();
      c1->cd();
      ...
      TString HNAME = DateTag+"_SignalEff_VBF_"+GetMassListVBF[i]+"_"+ID ;
