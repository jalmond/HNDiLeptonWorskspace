...
void DrawConfig(TString DateTag,TString Flavour, TString era, TString filepath,  TString ID){
  ...
  for (auto year : eras){
    ...
    TString path="/data6/Users/jalmond/SKFlatOutput/Run2UltraLegacy_v3/HNL_SignalRegion_Plotter/"+year+"/RunSignals__/";
    ...
    for(unsigned int i = 0 ; i < GetMassListDY.size(); i++){
      ...
      //h_eff_DY_SR2->Fill(GetMassListDY[i], EffSR2);
      //h_eff_DY_SR3->Fill(GetMassListDY[i], EffSR3);
