...
void CheckHPTPerc_Fake(){
  vector<TString> Etas = {"BB","EC"};
  vector<TString> Types = {"Prompt_EE","CF_EE","Conv_EE","Fake_EE"};
  for (auto ieta : Etas){
    for (auto itype : Types){
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_2017","Electron_pt_"+ieta+"_HNL_ULID_Defv8","Full ID","No Conv MVA ", ieta);
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Defv8","Electron_pt_"+ieta+"_HNL_ULID_Defv8_FO","Full ID","No Conv MVA ", ieta+"_FR_NoConv");
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_2017","Electron_pt_"+ieta+"_HNL_ULID_Defv2_FO","Full ID","No Conv MVA ", ieta+"_FR");
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Conv_Lone","Electron_pt_"+ieta+"_MVALoose","Full ID","No Conv MVA ", ieta+"_ConvOnly");
...
void DrawConfig(TString Era, TString Type, TString sNum,TString sDen,TString lNum, TString lDen, TString Eta){
  ...
  ratio_point->GetYaxis()->SetRangeUser(0,1.1);
