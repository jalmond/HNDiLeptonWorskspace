...
void CheckHPTPerc(){
  ...
  for (auto ieta : Etas){
    for (auto itype : Types){
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Split_1","Electron_pt_"+ieta+"_NoCut","Split_1 "+ieta,"GSF "+ieta);
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Split_2","Electron_pt_"+ieta+"_HNL_ULID_Probe_Split_2","Split_2 "+ieta,"TagID_Split_2 "+ieta);
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Split_3","Electron_pt_"+ieta+"_HNL_ULID_Probe_Split_3","Split_3 "+ieta,"TagID_Split_3 "+ieta);
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Split_4","Electron_pt_"+ieta+"_HNL_ULID_Probe_Split_4","Split_4 "+ieta,"TagID_Split_4 "+ieta);
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Split_5","Electron_pt_"+ieta+"_HNL_ULID_Probe_Split_5","Split_5 "+ieta,"TagID_Split_5 "+ieta);
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Split_6","Electron_pt_"+ieta+"_HNL_ULID_Probe_Split_6","Split_6 "+ieta,"TagID_Split_6 "+ieta);
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Split_7","Electron_pt_"+ieta+"_HNL_ULID_Probe_Split_7","Split_7 "+ieta,"TagID_Split_7 "+ieta);
      DrawConfig("2017",itype,"Electron_pt_"+ieta+"_HNL_ULID_Split_8","Electron_pt_"+ieta+"_HNL_ULID_Probe_Split_8","Split_8 "+ieta,"TagID_Split_8 "+ieta);
    }
  }  
...
void DrawConfig(TString Era, TString Type, TString sNum,TString sDen,TString lNum, TString lDen){
  ...
  HNL_Efficiency_Plotter Plotter("ElectronIDSF");
  Plotter.SetupPlotter(year,"","ElectronIDSF_split");
  ...
  TH1D *hist_num = Plotter.ConstructHist(path+"MC_DY_TT.root",Type+"/"+sNum);
  TH1D *hist_denom = Plotter.ConstructHist(path+"MC_DY_TT.root",Type+"/"+sDen);
  ...
  if(Type == "Prompt_EE")ratio_point->GetYaxis()->SetRangeUser(0.9,1.1);
  else ratio_point->GetYaxis()->SetRangeUser(0.5,1.5);
  if(ratio_point->GetBinContent(2) < 0.9) ratio_point->GetYaxis()->SetRangeUser(ratio_point->GetBinContent(2)*0.9,1.1);
  ...
  TString HNAME = Type + "_ElectronID_SF_Split__"+sNum +"_"+sDen ;
