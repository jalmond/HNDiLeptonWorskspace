...
void DrawConfig(TString DateTag,TString Flavour, TString era, TString filepath){
  ...
  for (auto year : eras){
    ...
    for(unsigned int i = 0 ; i < MassesDYS.size() ; i++){
      ...
      TH1D *hist_NumPresel     = Plotter.ConstructHist(path_mass,"ChannelCutFlow/"+ID+"/"+Flavour+"/ChannelDependant_SR3_HighMass");
    ...
    for(unsigned int i = 0 ; i < MassesDYS.size() ; i++){
      ...
      TH1D *hist_NumPresel     = Plotter.ConstructHist(path_mass,"ChannelCutFlow/"+ID+"/MuMu/ChannelDependant_SR3_HighMass");
    ...
    Plotter.draw_SignalEff_canvas(gPreDY,gPreSSWW,gPreDYMu,gPreSSWWMu,  Flavour+"_MuMu_DYEff",year,"HNL_ULID_ID_SR3_"+year);
