...
void DrawConfig(TString DateTag ,double mass, TString smass){
  ...
  Plotter.SetupPlotter(year,"","Muon_HighPt_TunevsGE");
  ...
  TH1D *hist_GE = Plotter.ConstructHist(path+"HNL_HighPtMuon_Studies_SkimTree_HNMultiLepBDT_HNL_M"+smass+"_private.root","SS/Muon1_Pt_Muon_ULID_v7");
  ...
  double int_highpt = hist_GE->Integral();
  ...
  TH1D *hist_empty = (TH1D*)hist_GE->Clone();
  ...
  if(hist_GE->GetMaximum() > hist_Tune->GetMaximum() )hist_empty->GetYaxis()->SetRangeUser(1,hist_empty->GetMaximum()*1.4);
  ...
  hist_GE->SetLineWidth(2.0);
  hist_GE->SetLineStyle(23);      
  hist_GE->SetLineStyle(4);
  hist_GE->Draw("histsame");
  ...
  lg->AddEntry(hist_GE,"GE Scale","l");
  ...
  c1_down->cd();
  TH1D *ratio_point = (TH1D *)hist_GE->Clone();
  ...
  ratio_point->GetYaxis()->SetTitle("#frac{GE}{TuneP}");
  ...
  TString HNAME = DateTag+"_SignalComp_TunePvsGE_"+ID +"_"+smass ;
