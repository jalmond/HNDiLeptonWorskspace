...
void DrawConfig(TString DateTag ,double mass, TString smass){
  ...
  Plotter.SetupPlotter(year,"","Muon_HighPt_TunevsRoch");
  ...
  TH1D *hist_Roch = Plotter.ConstructHist(path+"HNL_HighPtMuon_Studies_SkimTree_HNMultiLepBDT_HNL_M"+smass+"_private.root","SS/Muon1_Pt_Muon_ULID_v2");
  TH1D *hist_Tune = Plotter.ConstructHist(path+"HNL_HighPtMuon_Studies_SkimTree_HNMultiLepBDT_HNL_M"+smass+"_private.root","SS/Muon1_Pt_Muon_ULID_v5");

  //HNL_HighPtMuon_Studies_SkimTree_HNMultiLepBDT_DYTypeI_DF_M"+smass+"_private.root","SS/Muon1_Pt_Muon_ULID_v3");

  double int_highpt = hist_Roch->Integral();
  double int_tight = hist_Tune->Integral();
  ...
  TH1D *hist_empty = (TH1D*)hist_Roch->Clone();
  ...
  if(hist_Roch->GetMaximum() > hist_Tune->GetMaximum() )hist_empty->GetYaxis()->SetRangeUser(1,hist_empty->GetMaximum()*1.4);
  else hist_empty->GetYaxis()->SetRangeUser(1,hist_Tune->GetMaximum()*1.4);
  
  hist_empty->GetXaxis()->SetLabelSize(0);
  hist_empty->Draw("hist");


  hist_Roch->SetLineWidth(2.0);
  hist_Roch->SetLineStyle(23);      
  hist_Roch->SetLineStyle(4);
  hist_Roch->Draw("histsame");
  
  hist_Tune->SetLineColor(kRed);
  ...
  lg->SetBorderSize(0);
  lg->SetTextSize(0.03);
  lg->AddEntry(hist_Tune,"TuneP","l");
  lg->AddEntry(hist_Roch,"Rochester","l");
  ...
  c1_down->cd();
  TH1D *ratio_point = (TH1D *)hist_Tune->Clone();

  hist_axis(hist_empty, ratio_point);

  ratio_point->Divide(hist_Roch);
  ratio_point->GetYaxis()->SetTitle("#frac{TuneP}{Roch}");
  ...
  TString HNAME = DateTag+"_SignalComp_TunePvsRoch_"+ID +"_"+smass ;
