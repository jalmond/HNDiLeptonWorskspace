...
void CFRateEtaBins_Multi_BinEC(){
...
void CFRateEtaBinsIndiv(TString etabin){
  ...
  for (auto Era : Eras){
    ...
    if(!Era.Contains("2016")){
      hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.00001,0.1);
    }
    else{
      hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.00005,0.5);
    }
	...
	if(pt  < 400&& pt > 370) hist_eta1_Numerator->SetBinContent(i,0.0375);
	  //380.952 value = 0.0438075

	cout << pt << " value = " << hist_eta1_Numerator->GetBinContent(i) << endl;
      }
    }
	...
	if(pt  > 300)  hist_eta1_Numerator->SetBinContent(i,hist_eta1_Numerator->GetBinContent(i)*0.8);// 300
        else if(pt  > 100)    hist_eta1_Numerator->SetBinContent(i,hist_eta1_Numerator->GetBinContent(i)*0.9);
    ...
    SaveHistogram(Plotter, hists, {"1.556 < |#eta| < 2.5"}, "HNL_Lepton_ChargeFlip_"+ID+"_DATA_"+etabin, LabelForWeb, {Label});
