...
void CFRateEtaBinsIndiv(TString etabin){
  ...
  for (auto Era : Eras){
    ...
    if(!Era.Contains("2016")){
      hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.1);
    }
    else{
      hist_eta1_Numerator->GetYaxis()->SetRangeUser(0.000001,0.1);
    }
    ...
    SaveHistogram(Plotter, hists, {"1.1 < |#eta| < 1.444"}, "HNL_Lepton_ChargeFlip_"+ID+"_DATA_"+etabin, LabelForWeb, {Label});
