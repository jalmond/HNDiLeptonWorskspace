...
void PlotClosureCF_MET(){
  ...
  for(auto era : Eras) {
    ...
    vector<double> bins = {1};//4,8,12,16,20,24,28,32,36,40,44,48,52,56,64,72,80,100};
    
    //    RunClosure("Bin3","InvPtEta3", Plotter, "passHEEPID_v1", era, "Oct7","PBS","HNL_ChargeFlip_Closure_MET", "Separate_BinnedShift", "Closure_ElectronCF_MET" ,   bins);

    RunClosure("Bin3","InvPtEta3", Plotter, "HNL_ULID", era, "Oct8","PBS","HNL_ChargeFlip_Closure_MET", "Separate_BinnedShift", "Closure_ElectronCF_MET" ,   bins);
    //    RunClosure("Bin3","InvPtEta3", Plotter, "passIDHEEP_v3", era, "Oct8","PBS","HNL_ChargeFlip_Closure_MET", "Separate_BinnedShift", "Closure_ElectronCF_MET" ,   bins);
...
void RunClosure(TString Bin, TString Binning, HNLPlotter Plotter,TString ID, TString Era, TString  DateTag, TString Method, TString HistString, TString Indir,TString LabelForOutPut,vector<double> vrebin){
  ...
  /// Setup Input SS 
  TH1D *hist_mass_CF             = Plotter.ConstructHist(path,ID+"/MET/SS", vrebin);
  
  TH1D *hist_Prompt              = Plotter.ConstructHist(path,ID+"/"+Indir+"_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID+"/MET/OS_Corrected_EnergyShift", vrebin);
  if(!hist_mass_CF || !hist_Prompt){
    cout << path << endl;
    cout << ID+"/"+Indir+"_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID+"/MET/OS_Corrected_EnergyShift" << endl;
    cout << ID+"/"+Indir+"Up_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_PSigma_"+ID+"/MET/OS_Corrected_EnergyShift" << endl;
  }
  TH1D *hist_PromptUp              = Plotter.ConstructHist(path,ID+"/"+Indir+"Up_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_PSigma_"+ID+"/MET/OS_Corrected_EnergyShift", vrebin);
  TH1D *hist_PromptDown            = Plotter.ConstructHist(path,ID+"/"+Indir+"Down_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_MSigma_"+ID+"/MET/OS_Corrected_EnergyShift", vrebin);
  ...
  if(Method == "PBS") StatBinDown = "Separate_BinnedStatDown_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID;
   ...
   //Separate_BinnedStatUp_Bin2
  
  TH1D *hist_PromptSUp            = Plotter.ConstructHist(path,ID+"/"+StatBinUp+"/MET/OS_Corrected_EnergyShift", vrebin);
  TH1D *hist_PromptSDown          = Plotter.ConstructHist(path,ID+"/"+StatBinDown+"/MET/OS_Corrected_EnergyShift", vrebin);
  
  if(!hist_PromptSUp){
    cout << ID+"/"+StatBinUp+"/MET/OS_Corrected_EnergyShift" << endl;
  }
  ...
  hist_mass_CF->GetXaxis()->SetTitle("MET GeV ");
...
void SaveHistogram(TString Era,TString DateTag,HNLPlotter plotter,vector<TH1D*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> Labels){
  ...
  TLegend *lg = new TLegend(0.55, 0.60, 0.93, 0.85);
  lg->SetFillStyle(0);
  lg->SetBorderSize(0);
