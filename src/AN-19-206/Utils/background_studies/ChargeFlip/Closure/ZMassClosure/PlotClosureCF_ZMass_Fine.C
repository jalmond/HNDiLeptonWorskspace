...
void PlotClosureCF_ZMass_Fine(){
  ...
  for(auto era : Eras) {
    ...
    //    RunClosure("Bin3","InvPtEta3", Plotter, "HNL_ULID", era, "Oct7","CS","HNL_ChargeFlip_Closure_ZMass", "Paired_ConstShift", "Closure_ElectronCF_ZMass" ,  {2});
    ///    RunClosure("Bin3","InvPtEta3", Plotter, "passHEEPID_v1", era, "Oct1","PBS","HNL_ChargeFlip_Closure_ZMass", "Separate_BinnedShift", "Closure_ElectronCF_ZMass" ,  {50,60,70,80,85,90,95,100,110,120.,125.,130});
    //RunClosure("Bin3","InvPtEta3", Plotter, "passHEEPID_v3", era, "Oct1","PBS","HNL_ChargeFlip_Closure_ZMass", "Separate_BinnedShift", "Closure_ElectronCF_ZMass" ,  {50,60,70,80,85,90,95,100,110,120.,125.,130});

    RunClosure("Bin3","InvPtEta3", Plotter, "HNL_ULID", era, "Oct8","PBS","HNL_ChargeFlip_Closure_ZMass_Fine", "Separate_BinnedShift", "Closure_ElectronCF_ZMass" ,  {1});
    RunClosure("Bin3","InvPtEta3", Plotter, "passHEEPID_v3", era, "Oct8","PBS","HNL_ChargeFlip_Closure_ZMass_Fine", "Separate_BinnedShift", "Closure_ElectronCF_ZMass" ,  {1});
...
void RunClosure(TString Bin, TString Binning, HNLPlotter Plotter,TString ID, TString Era, TString  DateTag, TString Method, TString HistString, TString Indir,TString LabelForOutPut,vector<double> vrebin){
  ...
  TH1D *hist_Prompt              = Plotter.ConstructHist(path,ID+"/"+Indir+"_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID+"/ZMass/OS_Weighted_EnergyShift", vrebin);
  if(!hist_mass_CF || !hist_Prompt){
    cout << path << endl;
    cout << ID+"/"+Indir+"_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID+"/ZMass/OS_Weighted_EnergyShift" << endl;
    cout << ID+"/"+Indir+"Up_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_PSigma_"+ID+"/ZMass/OS_Weighted_EnergyShift" << endl;
  }
  TH1D *hist_PromptUp              = Plotter.ConstructHist(path,ID+"/"+Indir+"Up_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_PSigma_"+ID+"/ZMass/OS_Weighted_EnergyShift", vrebin);
  TH1D *hist_PromptDown            = Plotter.ConstructHist(path,ID+"/"+Indir+"Down_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_MSigma_"+ID+"/ZMass/OS_Weighted_EnergyShift", vrebin);
  
  TString StatBinUp   = Indir+"_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID;
  TString StatBinDown = Indir+"_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID;
  if(Method == "PBS") StatBinUp   = "Separate_BinnedStatUp_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID;
  if(Method == "PBS") StatBinDown = "Separate_BinnedStatDown_"+Bin+"/CFRate_"+Binning+"_"+HistLab+"_Central_"+ID;
  cout << StatBinUp << endl;
  ...
  double UpPrompt = hist_PromptUp->Integral() ; 
  double DownPrompt = hist_PromptDown->Integral() ; 
  double StatUpPrompt = hist_PromptSUp->Integral() ;
  double StatDownPrompt = hist_PromptSDown->Integral() ;

  cout << "Integrals " << Era << endl;
  cout << "Obs = " << hist_mass_CF->Integral() << endl;
  cout << "Pred = " << hist_Prompt->Integral() << " Smear [+ " << UpPrompt-hist_Prompt->Integral() << "  " << DownPrompt-hist_Prompt->Integral() <<  "] stat. [ "   << StatUpPrompt-hist_Prompt->Integral() << "  " << StatDownPrompt-hist_Prompt->Integral() <<  endl;

  
  SaveHistogram(Era,DateTag, Plotter,{hist_mass_CF,hist_Prompt,hist_PromptUp,hist_PromptDown,hist_PromptSUp,hist_PromptSDown}, {"SS Observed", "OS*R_{CF} Predicted","Pred Up","Pred Down"}, ID+"_CF_Closure_"+HistString+"_"+Method+"_"+Era, LabelForOutPut+"_"+Bin, {});
...
void SaveHistogram(TString Era,TString DateTag,HNLPlotter plotter,vector<TH1D*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> Labels){
  ...
  hist_default->GetXaxis()->SetRangeUser(75.,105.);
  ...
  hist_empty->GetXaxis()->SetRangeUser(75.,105.);
  ...
  TH1D *tmp_Up = (TH1D *)hists[2]->Clone();
  TH1D *tmp_Down = (TH1D *)hists[3]->Clone();
  TH1D *tmp_SUp = (TH1D *)hists[4]->Clone();
  TH1D *tmp_SDown = (TH1D *)hists[5]->Clone();
