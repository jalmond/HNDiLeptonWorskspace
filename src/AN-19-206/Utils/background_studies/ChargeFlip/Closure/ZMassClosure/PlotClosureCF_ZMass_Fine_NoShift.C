...
void PlotClosureCF_ZMass_Fine_NoShift(){
  ...
  for(auto era : Eras) {
    ...
    RunClosure("Bin3","InvPtEta3", Plotter, "passHEEPID_v3", era, "Oct8","PBS","HNL_ChargeFlip_Closure_ZMass", "Separate_BinnedShift", "Closure_ElectronCF_ZMass" ,  {50,60,70,80,85,90,95,100,110,120.,125.,130});

    RunClosure("Bin3","InvPtEta3", Plotter, "HNL_ULID", era, "Oct8","PBS","HNL_ChargeFlip_Closure_ZMass_Fine_NoShift", "Separate_BinnedShift", "Closure_ElectronCF_ZMass" ,  {1});
...
void RunClosure(TString Bin, TString Binning, HNLPlotter Plotter,TString ID, TString Era, TString  DateTag, TString Method, TString HistString, TString Indir,TString LabelForOutPut,vector<double> vrebin){
  ...
  TH1D *hist_Prompt            = Plotter.ConstructHist(path,ID+"/NoShift_"+Bin+"/CFRate_"+Binning+"_NoS_Central_"+ID+"/ZMass/OS_Weighted_EnergyShift" , vrebin);
  ...
  SaveHistogram(Era,DateTag, Plotter,{hist_mass_CF,hist_Prompt}, {"SS Observed", "OS*R_{CF} NoShift Predicted"}, ID+"_CF_Closure_"+HistString+"_"+Method+"_"+Era, LabelForOutPut+"_"+Bin, {});
