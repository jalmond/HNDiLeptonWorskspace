...
void PlotClosureCF_Pt(){
  ...
  for(auto era : Plotter.Eras()) {
    ...
    for(auto st : ShiftTypes){
      ...
      RunClosure(Plotter, "HNL_ULID_"+year, era, "May5","Closure","NonCF_LeptonPt_"+st,"HNL_ChargeFlip_Closure_Z_LeptonPt_"+st, {2});
...
void RunClosure(HNLPlotter Plotter,TString ID, TString Era, TString  DateTag,TString Method, TString HistString, TString LabelForOutPut,vector<double> vrebin){
  ...
  cout << path << endl;
  TH1D *hist_mass_CF             = Plotter.ConstructHist(path,ID+"/"+Method+"/CF_LeptonPt", vrebin);
  TH1D *hist_Prompt              = Plotter.ConstructHist(path,ID+"/"+Method+"/"+HistString, vrebin);

  TString HistStringNS = HistString;
  HistStringNS = HistStringNS.ReplaceAll("LeptonPt","LeptonPtNoShift");
