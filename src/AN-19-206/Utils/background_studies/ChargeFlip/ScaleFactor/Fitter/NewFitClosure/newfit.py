...
def RunFit(DateTag,channel, era, ID, HistName,fileName):
  ...
  filepath= os.getenv("FILE_MERGED_PATH") + "/HNL_Lepton_ChargeFlip/ScaleFactor/"+DateTag+"/"+era+"/ScaleFactor/HNL_Lepton_ChargeFlip_SkimTreeBDT_ScaleFactor.root"
  filepathMC= os.getenv("FILE_MERGED_PATH") + "/HNL_Lepton_ChargeFlip/ScaleFactor/"+DateTag+"/"+era+"/ScaleFactor/HNL_Lepton_ChargeFlip_SkimTree_DileptonBDT_DYJetsToEE_MiNNLO.root"
  print (filepath)
...
for Channel in Channels:
  for era in Eras:
    ...
    for Histx in Hists:
      RunFit("Oct8",Channel, era, HNLID,Histx,"FitResultsClosure")
      #RunFit("Oct8",Channel, era, "passHEEPID_v3",Histx,"FitResultsClosure")
