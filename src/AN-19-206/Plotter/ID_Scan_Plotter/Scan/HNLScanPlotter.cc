#include "HNLScanPlotter.h"
#include <stdlib.h>

HNLScanPlotter::HNLScanPlotter(TString macroname){
  ...
  LatexTextLabel_X=0.2;
  LatexTextLabel_Y=0.88;
...
void HNLScanPlotter::DrawStackPlots(){
  DrawDataAll(false);
  make_bkglist();
  draw_hist();
  return;
...
void HNLScanPlotter::DrawScanCutFlowWithData(TString defaultname){

  DrawDataAll(true);
  make_bkglist();
  draw_cutflow(defaultname);
  return;
}
void HNLScanPlotter::DrawStackCutFlowWithData(){
...
void HNLScanPlotter::DrawStackPlotsWithData(){
...
void HNLScanPlotter::SetupDefaultHist(TString sample, TString hn, TString histtag, TString legend, TString htype, TString hunit, vector<double> rb, double Xmin, double Xmax, double Ymax){
...
void HNLScanPlotter::AddCutFlow(TString histname){
...
void HNLScanPlotter::AddHist(TString hn, TString htype, TString hunit, vector<double> rb, double Xmin, double Xmax, double Ymax){
...
void HNLScanPlotter::BasicSetup(SetupHelper logy, SetupHelper ratio,  TString channel){
...
void HNLScanPlotter::SetupPlotter(TString era, TString Skim, TString Analyzer, TString FlagDir){
  SkimName=Skim;
  Era=era;
...
void HNLScanPlotter::SetupPlotter(TString FlagDir=""){
...
TString HNLScanPlotter::GetTitleByType(TString htype ){
...
TH1D* HNLScanPlotter::ConstructHist(TString filepath, TString fullhistname, bool DEBUGout){
...
TH1D* HNLScanPlotter::ConstructHist(TString filepath, TString fullhistname, vector<double> vrebinTMP,  bool DEBUGout){
...
void HNLScanPlotter::draw_hist(){
  
  cout << "Draw Stucture " << endl;
  for(int i = 0; i < HistNames.size(); i++) {
    for(int j = 0; j < HistPath.size(); j++){
      cout << "----> " << HistNames[i] << "  " << HistPath[j] << endl;
    }
  }
  cout << "Draw Hist() " << endl;
  ...
  for(i_var = 0; i_var < HistNames.size(); i_var++){
    ...
    // cout Add Plot Name to Dir path
    thiscut_plotpath = plotpath+ HistNames[i_var];
    mkdir(thiscut_plotpath);
    cout << "[1] making dir " << thiscut_plotpath << endl;
    cout << "Looping over i_cut " << i_cut << " / " << HistPath.size() << endl;
	...
	cout << "Skipping " << HistNames[i_var] << endl;
	continue;
      }
	...
	string shname = string(HPath2);
	istringstream iss(shname);
	string first_word="";
	iss >> first_word;
	thiscut_plotpath = thiscut_plotpath+"/"+TString(first_word);
	cout << "[2] making dir " << thiscut_plotpath << endl;
	mkdir(thiscut_plotpath);
      }
      
      cout << "draw_hist -----> " << thiscut_plotpath << " " << HPath << endl;
      cout << "n DefaultHists = " << DefaultHists.size() << " i_cut = " << i_cut << endl;
      TString defaultname = DefaultHists[i_cut];
      cout << "defaultname = " << defaultname << endl;
	...
	if(defaultname != "NULL"){
          if(!current_sample.Contains("NonPrompt")) DirName = defaultname;
          else DirName = temp_suffix;
          cout << "FAKE SCAN : " << current_sample << " DirName = " << DirName << endl;
	}
  ...
  if( HistPath[i_cut].Contains("High_OneFatJet") || histname[i_var].Contains("fj") ){
	...
	fill_legend(lg, hist_final);
      ...
      cout << "Deleting " <<  thiscut_plotpath +"/hists.root" << endl;
...
void HNLScanPlotter::draw_cutflow(TString defaultname){
  
  /// Loop over Cut/Regions set in Plotter.HistPat
  map<TString, double > GFMap;
  map<double,TString > GFMap2;
  for(i_cut = 0; i_cut < HistPath.size(); i_cut++){
    ...
    for(i_var = 0; i_var < HistNames.size(); i_var++){
      ...
      if(DrawSmallLegend)lg = new TLegend(0.55, 0.70, 0.93, 0.90);
	...
	cout << "ScaleBkg = " << endl;
	    ...
	    vector<double> binarr;
	    for(unsigned int i=1; i < hist_final->GetNbinsX()+2; i++)  binarr.push_back(hist_final->GetBinLowEdge(i));

	    MC_stacked_allerr = (TH1D*)hist_final->Clone("MC_stacked_allerr");
	    for(unsigned int i=1; i < MC_stacked_allerr->GetNbinsX()+1; i++) {
	      MC_stacked_allerr->SetBinContent(i, 0);
	      MC_stacked_allerr->SetBinError(i, 0);
	    }
	    MC_stacked_staterr = (TH1D*)hist_final->Clone("MC_stacked_staterr");
            for(unsigned int i=1; i < MC_stacked_staterr->GetNbinsX()+1; i++) {
	      MC_stacked_staterr->SetBinContent(i, 0);
	      MC_stacked_staterr->SetBinError(i, 0);
	    }
	  }
          hist_final->SetFillColor(map_sample_string_to_legendinfo[current_MCsector].second);
          hist_final->SetLineColor(map_sample_string_to_legendinfo[current_MCsector].second);
	  ...
	  //==== MC Norm Scaling
          ...
          else if( current_sample.Contains("CF") ) ThisSyst = 0.2;
          else{
            double mcnorm = 0.2;
	    double lumi =  0.04;///analysisInputs.CalculatedSysts["Luminosity"];
            ThisSyst = sqrt( mcnorm*mcnorm + lumi*lumi );
          ...
          for(int i=1; i<=n_bins; i++){
            ...
            double error_sumw2 = hist_final->GetBinError(i);
            double error_combined = sqrt( error_syst*error_syst + error_sumw2*error_sumw2 );
	    cout << current_sample << " Bin " << i << " Error " << error_combined/hist_final->GetBinContent(i) << endl;
            if(hist_final->GetBinContent(i) > 0)hist_final->SetBinError(i, error_combined);
	    else hist_final->SetBinError(i,0);
          }
	  
          MC_stacked->Add(hist_final);
          MC_stacked_allerr->Add(hist_final);
	  for(unsigned int i=1; i < MC_stacked_staterr->GetNbinsX()+1; i++){
	    cout << "Added MC_stacked_allerr Bin " << i << " Get Bin Content " << MC_stacked_allerr->GetBinContent(i) <<  " error = " << MC_stacked_allerr->GetBinError(i)/MC_stacked_allerr->GetBinContent(i) <<  endl;
	  }
	...
	fill_legend(lg, hist_final);
      ...
      for(unsigned int i=1; i < MC_stacked_staterr->GetNbinsX()+1; i++){
	cout << "MC_stacked_staterr Bin " << i << " Get Bin Content " << MC_stacked_staterr->GetBinContent(i) << " error = " << MC_stacked_staterr->GetBinError(i)/MC_stacked_staterr->GetBinContent(i) <<  endl;
	cout << "MC_stacked_allerr Bin " << i << " Get Bin Content " << MC_stacked_allerr->GetBinContent(i) <<  " error = " << MC_stacked_allerr->GetBinError(i)/MC_stacked_allerr->GetBinContent(i) <<  endl;
      }
      
      double VAL = draw_canvas_fakescan(MC_stacked, MC_stacked_staterr, MC_stacked_allerr, hist_data, hist_signal, lg, drawdata.at(i_cut), outputfile,temp_suffix);
      GFMap[temp_suffix] = VAL;
      GFMap2[VAL] = temp_suffix;
      cout << "ADDING TO MAP " << temp_suffix << " " << VAL  << endl;
      //==== legend is already deleted in draw_canvas()
      //delete lg; 
  ...
  cout << "Run over Fitness results" << endl;
  ...
  for(auto i: GFMap){
    if(i.first==sFit)    cout << i.first << " " << i.second << " ------------> OPTIMAL "<< endl;
    else cout << i.first << " " << i.second << endl;
  }
  for(auto i: GFMap2)cout << "Ordered " << i.first << " " << i.second << endl;
   

}

void HNLScanPlotter::make_bkglist(){
...
void HNLScanPlotter::SetRebins(TString filepath){

  cout << "[HNLScanPlotter::SetRebins] Get rebins from " << filepath << endl;
...
void HNLScanPlotter::SetYAxis(TString filepath){

  cout << "[HNLScanPlotter::SetYAxis] Get Yaxis from " << filepath << endl;
...
void HNLScanPlotter::SetXAxis(TString filepath){

  cout << "[HNLScanPlotter::SetXAxis] Get Xaxis from " << filepath << endl;
...
vector<double> HNLScanPlotter::GetRebinVariableBins(){
...
void HNLScanPlotter::MakeRebins(){
...
TString HNLScanPlotter::find_MCsector(){
...
void HNLScanPlotter::clear_legend_info(){
...
double HNLScanPlotter::coupling_constant(int mass){
...
void HNLScanPlotter::fill_legend(TLegend* lg, TH1D* hist){
...
void HNLScanPlotter::draw_legend(TLegend* lg, bool DrawData){
  // Example :
  //                      0    1    2    3
...
void HNLScanPlotter::draw_comp_canvas(TH1D *hist_def, TH1D *hist_comp, TLegend *legend ,TFile *outputf){
  ...
  TString YTitle;
  cout << "Rebins size = " << Rebins.size() << " Rebins[0].size() = " << Rebins[0].size() << " val = " << Rebins[0][0] << endl;
  ...
  if(x_title[0] != "") ratio_allerr->GetXaxis()->SetTitle(x_title[0]);
...
double HNLScanPlotter::draw_canvas_fakescan(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> hist_signal, TLegend *legend, bool DrawData, TFile *outputf, TString TAG){
  ...
  if(!drawratio.at(i_cut)){
	...
	NUM = abs((ratio_point->GetBinContent(i) / mc_allerror->GetBinContent(i)) - 1);
	  ...
	  DEN = (err_up_tmp.at(i-1) /mc_allerror->GetBinContent(i))*(err_up_tmp.at(i-1) /mc_allerror->GetBinContent(i));
        }
        else{
          ...
          gr_ratio_point->SetPointEXhigh(i-1, 0);
	  DEN = (1.8 / mc_allerror->GetBinContent(i))*(1.8 / mc_allerror->GetBinContent(i));
        }
	...
	DEN+= (mc_allerror->GetBinError(i)/ mc_allerror->GetBinContent(i))*(mc_allerror->GetBinError(i)/ mc_allerror->GetBinContent(i));
  ...
  if(!MakePaperPlot){
    TLatex latex_CMSPriliminary, latex_Lumi,latex_TAG;
    latex_CMSPriliminary.SetNDC();
    latex_Lumi.SetNDC();
    latex_TAG.SetNDC();
    latex_CMSPriliminary.SetTextSize(LatexTextCMS_Size);
    ...
    latex_TAG.SetTextSize(0.025);
    TAG=TAG.ReplaceAll("ChannelCutFlow/HNL_ULID_","");
    //latex_TAG.DrawLatex(LatexTextLabel_X,LatexTextLabel_Y -0.05,TAG);

    //    std::string trimmedString = std::to_string(FITNESSTEST).substr(0, std::to_string(FITNESSTEST).find(".") + 2 + 1);
    
    //    latex_TAG.DrawLatex(LatexTextLabel_X,LatexTextLabel_Y -0.1,"GF="+TString(trimmedString));
  }
  else{
    //==== This is for Paper                                                                                                                                                                                                           
    TLatex latex_CMSPriliminary, latex_Lumi;
  ...
  if(!RunScan) c1->SaveAs(thiscut_plotpath+"/"+HistNames[i_var]+".pdf");
  else{
    TString SaveName =  thiscut_plotpath+"/"+HistNames[i_var]+".pdf";
    SaveName =SaveName.ReplaceAll("AJ30/","AJ30_");
...
void HNLScanPlotter::draw_canvas(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> hist_signal, TLegend *legend, bool DrawData, TFile *outputf){

  if(!hist_data) return;

  cout << "draw_canvas ---> " + thiscut_plotpath+"  _ " +HistNames[i_var] << endl;

  for(unsigned int i=1; i < mc_staterror->GetNbinsX()+1; i++)   cout << "PreBin " << i << " Get Bin Content " << mc_staterror->GetBinContent(i) << endl;
  ...
  double DataInt = hist_data->Integral(0,hist_data->GetNbinsX()+1);

  double BkgInt = 0;
  TList *list_stack2 = mc_stack->GetHists();
  for(int i=0; i < list_stack2->Capacity(); i++){
    TH1D *this_hist = (TH1D *)list_stack2->At(i);
    BkgInt+= this_hist ->Integral(0,this_hist->GetNbinsX()+1);
  }
  ...
  if(!MakePaperPlot){
    ...
    TString str_channel = GetStringChannelRegion(LeptonChannels.at(i_cut), "");
    TString str_data = "N_{Data}=" +DToString(DataInt);
    TString str_bkg = "N_{bkg}=" +DToString(BkgInt);
    ...
    channelname.DrawLatex(LatexTextLabel_X, LatexTextLabel_Y-0.04, str_data);
    channelname.DrawLatex(LatexTextLabel_X, LatexTextLabel_Y-0.08, str_bkg);
  ...
  //  if(!RunScan)mkdir(thiscut_plotpath);

  TString HNAME_FIX = HistNames[i_var];
  cout << "HNAME_FIX = " << HNAME_FIX << endl;
  if(HNAME_FIX.Contains("/")){
    ...
    HNAME_FIX = HNAME_FIX.ReplaceAll(first_word+"/","");
  }
  
  TString HNAME_FIX2 = HistPath[i_cut];
  if(HNAME_FIX2.Contains("/")){

    TString HNAME = HistPath[i_cut];
    HNAME = HNAME.ReplaceAll("/"," ");
  ...
  cout << "Saving -> " << thiscut_plotpath+"/"+HNAME_FIX+"_"+HNAME_FIX2+".pdf" << endl;
...
void HNLScanPlotter::SetXaxisRange(TH1D* hist){
...
void HNLScanPlotter::SetXaxisRange(THStack* mc_stack){
...
void HNLScanPlotter::SetXaxisRangeBoth(THStack* mc_stack, TH1D* hist){
...
TH1D* HNLScanPlotter::MakeOverflowBin(TH1D* hist){
...
TString HNLScanPlotter::DoubleToString(double dx){

  //cout << "[HNLScanPlotter::DoubleToString] var = " << HistNames[i_var] << endl;
  //cout << "[HNLScanPlotter::DoubleToString] unit = " << units[i_var] << endl;
  //cout << "[HNLScanPlotter::DoubleToString] dx = " << dx << endl;
...
TString HNLScanPlotter::legend_coupling_label(int mass){
...
void HNLScanPlotter::mkdir(TString path){
...
double HNLScanPlotter::GetHistValue(TH1D * ht, TString hn){
...
double HNLScanPlotter::GetHistError(TH1D * ht, TString hn){
...
TString HNLScanPlotter::FixLatex(TString origSt){
  TString fixSt=origSt;
  if(fixSt.Contains("P_T")) fixSt.ReplaceAll("P_T","$\\mathrm{P}_{T}$");
  return fixSt;
}
void HNLScanPlotter::MakeTexFile(map< TString, TH1D * > hs, TString Hist_For_CutFlow){
...
void HNLScanPlotter::Summary(){
  
  cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
  cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
  cout << " "<< endl;
  cout << "HNLScanPlotter Ran StackPlots : With Data. Results sent to : " << endl;
...
TString HNLScanPlotter::GetStringChannelRegion(int A, TString B){
...
bool HNLScanPlotter::ZeroDataCheckCut(double xlow, double xhigh){
...
void HNLScanPlotter::ScaleSample(TString samplename, double scale_value){

  map_sample_string_to_scale[samplename]=scale_value;
  

}
double HNLScanPlotter::GetScale(TString samplename){
...
void HNLScanPlotter::SetupSampleInfo(){
  ...
  map_sample_string_to_list["VV_incl"] = {"WZ_pythia", "ZZ_pythia", "WWTo2L2Nu_powheg"};
  map_sample_string_to_list["WZ1_Trig"] = {"Prompt_TrigSF_WZTo3LNu_mllmin4p0_powheg"};
  map_sample_string_to_list["WZ2_Trig"] = {"Prompt_TrigSF_WZ_pythia"};
  map_sample_string_to_list["WZ3_Trig"] = {"Prompt_TrigSF_WZTo3LNu_amcatnlo"};
  map_sample_string_to_list["WZ1_NoIDSF"] = {"Prompt_NoIDSF_WZTo3LNu_mllmin4p0_powheg"};
  map_sample_string_to_list["WZ2_NoIDSF"] = {"Prompt_NoIDSF_WZ_pythia"};
  map_sample_string_to_list["WZ3_NoIDSF"] = {"Prompt_NoIDSF_WZTo3LNu_amcatnlo"};
  ...
  map_sample_string_to_list["OSOther"] = {"WZ_pythia", "ZZ_pythia"};
  ...
  map_sample_string_to_legendinfo["WZ1_NoTrig"] = make_pair("WZ", kYellow);
  map_sample_string_to_legendinfo["WZ2_NoTrig"] = make_pair("WZ", kYellow);
  map_sample_string_to_legendinfo["WZ3_NoTrig"] = make_pair("WZ", kYellow);
  map_sample_string_to_legendinfo["WZ1_Standard"] = make_pair("WZ", kYellow);
  map_sample_string_to_legendinfo["WZ2_Standard"] = make_pair("WZ", kYellow);
  map_sample_string_to_legendinfo["WZ3_Standard"] = make_pair("WZ", kYellow);
  ...
  map_sample_string_to_legendinfo["OSOther"] = make_pair("Other", kRed);
...
void HNLScanPlotter::LeptonChannel(TString ch){
...
void HNLScanPlotter::UseLogyAll(double v){
  UseLogy.clear();
  for(int i=0; i < HistPath.size() ; i++) UseLogy.push_back(v);
}

void HNLScanPlotter::ApplyMCNormSFAll(bool v){
  ApplyMCNormSF.clear();
  for(int i=0; i < HistPath.size()  ; i++)ApplyMCNormSF.push_back(v);
}
void HNLScanPlotter::DrawRatioAll(bool v){
...
void HNLScanPlotter::DrawDataAll(bool v){

  drawdata.clear();
  for(int i=0; i < HistPath.size()  ; i++) drawdata.push_back(v);
}

vector<double> HNLScanPlotter::GetRebinZeroBackground(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> &hist_signal){
...
TString HNLScanPlotter::Scan2DHists(TH2D* h1, TH2D* h2){
...
TH1D* HNLScanPlotter::GetCutEfficiency(TH1D *hist_default){
...
TH1D* HNLScanPlotter::GetScanEfficiency(TH1D *hist_default){
...
TH1D* HNLScanPlotter::GetScanEfficiency2D(TH1D *hist_default, TH1D *hist_comp){
...
void HNLScanPlotter::Draw_Graph(vector<TGraph*> vgr , vector<TString> legNames,  TString HistName,TString dirName, vector<TString> tlat){

  thiscut_plotpath = plotpath+"/"+ dirName;
  mkdir(thiscut_plotpath);

  TLegend *lg= new TLegend(Legend_X1,Legend_Y1,Legend_X2,Legend_Y2);
  lg->SetFillStyle(0);
  lg->SetBorderSize(0);
  ...
  hist_empty->GetYaxis()->SetTitle(YAxisTitle);
  hist_empty->GetXaxis()->SetTitle(XAxisTitle);
  hist_empty->GetYaxis()->SetRangeUser(default_y_min+0.01, Ymax*Hist_YAxis_MaxScale);
  hist_axis(hist_empty);
  ...
  for(unsigned int ig=1; ig < vgr.size(); ig++){
    ...
    gr->SetLineWidth(4.0);
    gr->SetMarkerSize(0.);
    gr->SetLineColor(HNLScanPlotter::GetColor(ig));
    gr->Draw("plsame");
  ...
  hist_empty->GetYaxis()->SetRangeUser(default_y_min, Ymax*Hist_YAxis_MaxScale);

  TLatex latex_result;
  latex_result.SetNDC();
  latex_result.SetTextSize(LatexTextLabel_Size);
  //  latex_result.DrawLatex(LatexTextLabel_X, LatexTextLabel_Y,HistName);
  ...
  for(unsigned int il =0 ; il < tlat.size(); il++) latex_result.DrawLatex(LatexTextLabel_X, LatexTextLabel_Y-0.1 - 0.05*(il), tlat[il]);
...
void HNLScanPlotter::draw_hists_canvas(vector<TH1D*> hists , vector<TString> legNames,  TString HistName,TString dirName){
  ...
  hist_empty->GetYaxis()->SetRangeUser(0., Hist_YAxis_MaxScale);
  hist_axis(hist_empty);
  ...
  for(int ig =1; ig < hists.size(); ig++){
    ...
    gr->SetMarkerSize(0.);
    if(ig < hists.size() /2) gr->SetLineColor(HNLScanPlotter::GetColor(ig));
    else gr->SetLineColor(HNLScanPlotter::GetColor(ig- (hists.size() /2)));
    gr->Draw("plsame");
...
void HNLScanPlotter::draw_hists_canvas_pt(vector<TH1D*> hists , vector<TString> legNames,  TString HistName,TString dirName){
  ...
  TGraphAsymmErrors *gr1 = new TGraphAsymmErrors(hist_default);
  gr1->SetLineWidth(2.0);
  gr1->SetMarkerColor(HNLScanPlotter::GetColor(0));
  gr1->SetLineColor(kRed);
  gr1->Draw("pl0same");
  lg->AddEntry(gr1, legNames[0], "pl");
  ...
  for(int ig =1; ig < hists.size(); ig++){
    ...
    //    gr->SetMarkerSize(0.);
    gr->SetMarkerColor(HNLScanPlotter::GetColor(ig+1));
    gr->SetLineColor(HNLScanPlotter::GetColor(ig+1));
    gr->Draw("pl0same");
...
double  HNLScanPlotter::draw_SvsB_canvas(vector<TH1D*> hists , vector<TString> IDs, vector<TString> legNames,  TString HistName, TString dirName){
...
void HNLScanPlotter::MakeAUCHist(map<TString,double> AUCMap,TString HistName,TString dirName, TString Label){
...
void HNLScanPlotter::SaveHist(TH1D* hist, TString legname, TString HistName, TString dirName){
  ...
  hist_axis(hist_empty);
  hist_empty->GetYaxis()->SetRangeUser(0, 1.);//Hist_YAxis_MaxScale);
...
TString HNLScanPlotter::SaveProfile(double Cut, vector<TProfile*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> scales){
  ...
  double Ymin = default_y_min+0.000001;
  double YmaxScale =0.000001;

  for(auto i : hists)  if(i->GetMaximum() > YmaxScale) YmaxScale = i->GetMaximum()*Hist_YAxis_MaxScale;

  if(default_y_max > 0) YmaxScale = default_y_max;
  ...
  for(int i=0 ; i < hists.size(); i++){
    hists[i]->SetLineColor(HNLScanPlotter::GetColor(i));
...
void HNLScanPlotter::SaveHists(vector<TH1D*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> scales, bool drawError){
  ...
  for(auto i : hists) {
    if(i->GetMaximum() > YmaxScale) YmaxScale = i->GetMaximum()*Hist_YAxis_MaxScale;
  }
  hist_axis(hist_empty);
  ...
  if(XaxisMin != -999) hist_empty->GetXaxis()->SetRangeUser(XaxisMin, XaxisMax);
...
double HNLScanPlotter::GetCutFromEff(TH2D* h , int xbin, double WP){
...
Double_t HNLScanPlotter::fitf(Double_t *x,Double_t *par) {
...
void HNLScanPlotter::Draw_MVA_WP(vector<TH2D*>hists, vector<TString> histsname,  double WP,TString HistName, TString dirName){
  ...
  hist_empty->GetYaxis()->SetRangeUser(-1., Hist_YAxis_MaxScale);
...
void HNLScanPlotter::SaveHistsWithRatio(vector<TH1D*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> scales){
...
void  HNLScanPlotter::DrawEfficiency( vector<TH1D*> hists, vector<TString> legNames, TString HistName, TString dirName){
...
void HNLScanPlotter::draw_hist_canvas(TH1D *hist_default,  TString HistName){
