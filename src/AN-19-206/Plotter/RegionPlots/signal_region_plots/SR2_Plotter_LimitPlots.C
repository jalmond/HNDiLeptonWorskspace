...
void SR2_Plotter_LimitPlots(){
  ...
  SSSignalRegionJob(DATE, Eras, "MuMu",{"Prompt","NonPrompt","Conv"} , Reg , "HNL_SignalRegion_SSCRPlots_LimitExtraction", "List3");
...
void RunFunction(TString DateFileTag,vector<TString> eras,vector<TString> channels, TString PlotterTag, TString Flag,vector<TString> Bkgs, TString Region, TString InputList){
  ...
  for (auto year : eras){
    for (auto channel : channels){
      ...
      ///// HISTs Setup
      Plotter.AddHist("LimitShape_SR2/HT_PT_Central"  ,"Search Region"  , "",    {1}, 0, 2000);
      
      Plotter.signal_mass = {1000};
      Plotter.signal_scale = {0.1};

      TString V2 = "#||{V_{N#mu}}^{2}";                                                                                                                                       
      Plotter.signal_leg = {"m_{N} = 1000 GeV, "+V2+"=0.1"};
      Plotter.signal_color = {kBlue};
      Plotter.signal_string = {"SSWW_MN1000"};
