#include "HNLSRPlotter.h"
#include <stdlib.h>

HNLSRPlotter::HNLSRPlotter(TString macroname){
...
void HNLSRPlotter::DrawStackPlots(){
  DrawDataAll(false);
  make_bkglist();
  draw_hist();
  return;
...
void HNLSRPlotter::DrawStackCutFlowWithData(){
...
void HNLSRPlotter::DrawStackPlotsWithData(){
...
void HNLSRPlotter::DrawComparisonPlots(){
  if(!comp_default_set) return;
  if(FullHistNames.size() ==0) return;
  draw_comphist();
}

void HNLSRPlotter::SetupDefaultHist(TString sample, TString hn, TString histtag, TString legend, TString htype, TString hunit, vector<double> rb, double Xmin, double Xmax, double Ymax){
...
void HNLSRPlotter::SetupComparisonHist(TString sample, TString hist, TString histtag, TString legend){
...
void HNLSRPlotter::AddCutFlow(TString histname){
...
void HNLSRPlotter::AddHist(TString hn, TString htype, TString hunit, vector<double> rb, double Xmin, double Xmax, double Ymax){
...
void HNLSRPlotter::BasicSetup(SetupHelper logy, SetupHelper ratio,  TString channel){
...
void HNLSRPlotter::SetupPlotter(TString era, TString Skim, TString Analyzer, TString FlagDir){
  SkimName=Skim;
  Era=era;
...
void HNLSRPlotter::SetupPlotter(TString FlagDir=""){
...
TString HNLSRPlotter::GetTitleByType(TString htype ){
...
TH1D* HNLSRPlotter::ConstructHist(TString filepath, TString fullhistname, bool DEBUGout){
...
TH1D* HNLSRPlotter::ConstructHist(TString filepath, TString fullhistname, vector<double> vrebinTMP,  bool DEBUGout){
...
TH2D* HNLSRPlotter::Construct2DHist(TString filepath, TString fullhistname){
...
TH1D* HNLSRPlotter::MakeHist(TString filepath, TString fullhistname){
...
void HNLSRPlotter::draw_comphist(){
...
void HNLSRPlotter::draw_hist(){
  ...
  for(i_cut = 0; i_cut < HistPath.size(); i_cut++){
    ...
    for(i_var = 0; i_var < HistNames.size(); i_var++){
      ...
      if(drawratio.at(i_cut)){
        ...
        //==== SR
        else lg = new TLegend(0.60, 0.45, 0.96, 0.92);
      }
      else{
        //==== CR
      ...
      cout << "signal_mass size = " << signal_mass.size () << endl;
	  ...
	  cout << "signal_index = " << signal_index << " => mass = " << signal_mass[signal_index] << endl;
          ...
          if(HistPath[i_cut].Contains("EE")) WhichChannel = "ElEl";
          if(HistPath[i_cut].Contains("EMu")) WhichChannel = "MuEl";

          TString string_signal_mass = TString::Itoa(abs(signal_mass[signal_index]),10);
	  
          filepath = infilepath + "/SIG/"+filename_prefix+"_"+SkimName+"_"+signal_string[signal_index]+".root";
          current_sample = signal_string[signal_index];
	  cout << "filepath SIG = " << filepath << endl;
        }
	...
	cout << "Moving to dir " << DirName << endl;
	...
	*/
	...
	cout << "Scale " << bkglist[i_file]  << " " <<  ScaleBkg<< endl;
  ...
  if( HistPath[i_cut].Contains("High_OneFatJet") || histname[i_var].Contains("fj") ){
	  ...
	  */
	  cout << "SCALING SIGNAL " << k_factor*this_coupling_constant/(1.*TMath::Power(10,log_of_generation_mixing))  << endl;
          //hist_final->Scale( k_factor*this_coupling_constant/(1.*TMath::Power(10,log_of_generation_mixing)) );
	  hist_final->Scale( signal_scale[signal_index]);
	...
	fill_legend(lg, hist_final);
      ...
      cout << "hist_signal = " << hist_signal.at(0)->Integral() << endl;
...
void HNLSRPlotter::draw_cutflow(){
...
void HNLSRPlotter::make_bkglist(){
...
void HNLSRPlotter::SetRebins(TString filepath){

  cout << "[HNLSRPlotter::SetRebins] Get rebins from " << filepath << endl;
...
void HNLSRPlotter::SetYAxis(TString filepath){

  cout << "[HNLSRPlotter::SetYAxis] Get Yaxis from " << filepath << endl;
...
void HNLSRPlotter::SetXAxis(TString filepath){

  cout << "[HNLSRPlotter::SetXAxis] Get Xaxis from " << filepath << endl;
...
vector<double> HNLSRPlotter::GetRebinVariableBins(){
...
TString HNLSRPlotter::find_MCsector(){
...
void HNLSRPlotter::clear_legend_info(){
...
double HNLSRPlotter::coupling_constant(int mass){
...
void HNLSRPlotter::fill_legend(TLegend* lg, TH1D* hist){
...
void HNLSRPlotter::draw_legend(TLegend* lg, bool DrawData){
  // Example :
  //                      0    1    2    3
  ...
  if(CurrentSC==no_class){
    for(unsigned int i=0; i<signal_survive_mass.size(); i++){
      ...
      cout << "this_mass = " << this_mass << endl;
      if( find(signal_mass.begin(), signal_mass.end(), this_mass) != signal_mass.end()){
	
        lg->AddEntry(hist_for_legend_signal.at(i), legend_coupling_label(i), "l");
      }
    }
...
void HNLSRPlotter::draw_comp_canvas(TH1D *hist_def, TH1D *hist_comp, TLegend *legend ,TFile *outputf){
  ...
  ///  if(x_title[0] != "") ratio_allerr->GetXaxis()->SetTitle(x_title[0]); FIX
...
void HNLSRPlotter::draw_canvas(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> hist_signal, TLegend *legend, bool DrawData, TFile *outputf){
  ...
  //==== Ratio
  cout << "drawratio " << drawratio.size() << " " << i_cut <<endl;

  if(!drawratio.at(i_cut)){
    hist_axis(hist_empty);
    if(x_title[i_var] != "")hist_empty->GetXaxis()->SetTitle(x_title[i_var]);
  }
  else{
    //==== MC-DATA
    c1_down->cd();
    ...
    if(x_title[i_var] != ""){
      ...
      if(x_title[i_var] != "")ratio_allerr->GetXaxis()->SetTitle(x_title[i_var]);
    }
...
void HNLSRPlotter::SetXaxisRange(TH1D* hist){
...
void HNLSRPlotter::SetXaxisRange(THStack* mc_stack){
...
void HNLSRPlotter::SetXaxisRangeBoth(THStack* mc_stack, TH1D* hist){
...
TH1D* HNLSRPlotter::MakeOverflowBin(TH1D* hist){
...
TString HNLSRPlotter::DoubleToString(double dx){

  //cout << "[HNLSRPlotter::DoubleToString] var = " << HistNames[i_var] << endl;
  //cout << "[HNLSRPlotter::DoubleToString] unit = " << units[i_var] << endl;
  //cout << "[HNLSRPlotter::DoubleToString] dx = " << dx << endl;
...
TString HNLSRPlotter::legend_coupling_label(int mass){
...
void HNLSRPlotter::mkdir(TString path){
...
double HNLSRPlotter::GetHistValue(TH1D * ht, TString hn){
...
double HNLSRPlotter::GetHistError(TH1D * ht, TString hn){
...
TString HNLSRPlotter::FixLatex(TString origSt){
  TString fixSt=origSt;
  if(fixSt.Contains("P_T")) fixSt.ReplaceAll("P_T","$\\mathrm{P}_{T}$");
  return fixSt;
}
void HNLSRPlotter::MakeTexFile(map< TString, TH1D * > hs, TString Hist_For_CutFlow){
...
void HNLSRPlotter::Summary(){
  
  cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
  cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
  cout << " "<< endl;
  cout << "HNLSRPlotter Ran StackPlots : With Data. Results sent to : " << endl;
...
TString HNLSRPlotter::GetStringChannelRegion(int A, TString B){
  ...
  if(A==40) channel = "4l";
  if(A==26) channel = "eee";
  if(A==27) channel = "emull";
  if(A==28) channel = "eeee";
  if(A==29) channel = "#mu#mu#mu#mu";
...
bool HNLSRPlotter::ZeroDataCheckCut(double xlow, double xhigh){
...
void HNLSRPlotter::ScaleSample(TString samplename, double scale_value){

  map_sample_string_to_scale[samplename]=scale_value;
  

}
double HNLSRPlotter::GetScale(TString samplename){
...
void HNLSRPlotter::SetupSampleInfo(){
  ...
  map_sample_string_to_legendinfo["WW_SS"] = make_pair("W^{+}W^{+}", kYellow);
  map_sample_string_to_legendinfo["ZZ_excl"] = make_pair("ZZ", kRed-7);
  map_sample_string_to_legendinfo["ZZ_SS"] = make_pair("ZZ", kRed-7);
  ...
  map_sample_string_to_legendinfo["Other"] = make_pair("Other", kRed);
  map_sample_string_to_legendinfo["Other_SS"] = make_pair("Other", kOrange);
...
void HNLSRPlotter::LeptonChannel(TString ch){
...
void HNLSRPlotter::UseLogyAll(double v){
  UseLogy.clear();
  for(int i=0; i < HistPath.size() ; i++) UseLogy.push_back(v);
}

void HNLSRPlotter::ApplyMCNormSFAll(bool v){
  ApplyMCNormSF.clear();
  for(int i=0; i < HistPath.size()  ; i++)ApplyMCNormSF.push_back(v);
}
void HNLSRPlotter::DrawRatioAll(bool v){
...
void HNLSRPlotter::DrawDataAll(bool v){

  drawdata.clear();
  for(int i=0; i < HistPath.size()  ; i++) drawdata.push_back(v);
}

vector<double> HNLSRPlotter::GetRebinZeroBackground(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> &hist_signal){
...
TString HNLSRPlotter::Scan2DHists(TH2D* h1, TH2D* h2){
...
TH1D* HNLSRPlotter::GetCutEfficiency(TH1D *hist_default){
...
TH1D* HNLSRPlotter::GetScanEfficiency(TH1D *hist_default){
...
TH1D* HNLSRPlotter::GetScanEfficiency2D(TH1D *hist_default, TH1D *hist_comp){
...
void HNLSRPlotter::Draw_Graph(vector<TGraph*> vgr , vector<TString> legNames,  TString HistName,TString dirName, vector<TString> tlat){
  ...
  if(XAxisTitle != "")hist_empty->GetXaxis()->SetTitle(XAxisTitle);
  ...
  for(unsigned int ig=1; ig < vgr.size(); ig++){
    ...
    gr->SetLineWidth(4.0);
    gr->SetMarkerSize(0.);
    gr->SetLineColor(HNLSRPlotter::GetColor(ig));
    gr->Draw("plsame");
...
void HNLSRPlotter::draw_hists_canvas(vector<TH1D*> hists , vector<TString> legNames,  TString HistName,TString dirName){
  ...
  for(int ig =1; ig < hists.size(); ig++){
    ...
    gr->SetMarkerSize(0.);
    if(ig < hists.size() /2) gr->SetLineColor(HNLSRPlotter::GetColor(ig));
    else gr->SetLineColor(HNLSRPlotter::GetColor(ig- (hists.size() /2)));
    gr->Draw("plsame");
...
void HNLSRPlotter::draw_hists_canvas_pt(vector<TH1D*> hists , vector<TString> legNames,  TString HistName,TString dirName){
  ...
  TGraphAsymmErrors *gr1 = new TGraphAsymmErrors(hist_default);
  gr1->SetLineWidth(2.0);
  gr1->SetMarkerColor(HNLSRPlotter::GetColor(0));
  gr1->SetLineColor(kRed);
  gr1->Draw("pl0same");
  lg->AddEntry(gr1, legNames[0], "pl");
  ...
  for(int ig =1; ig < hists.size(); ig++){
    ...
    //    gr->SetMarkerSize(0.);
    gr->SetMarkerColor(HNLSRPlotter::GetColor(ig+1));
    gr->SetLineColor(HNLSRPlotter::GetColor(ig+1));
    gr->Draw("pl0same");
...
double  HNLSRPlotter::draw_SvsB_canvas(vector<TH1D*> hists , vector<TString> IDs, vector<TString> legNames,  TString HistName, TString dirName){
...
void HNLSRPlotter::MakeAUCHist(map<TString,double> AUCMap,TString HistName,TString dirName, TString Label){
...
void HNLSRPlotter::SaveHist(TH1D* hist, TString legname, TString HistName, TString dirName){
...
TString HNLSRPlotter::SaveProfile(double Cut, vector<TProfile*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> scales){
  ...
  for(int i=0 ; i < hists.size(); i++){
    hists[i]->SetLineColor(HNLSRPlotter::GetColor(i));
...
void HNLSRPlotter::SaveHists(vector<TH1D*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> scales, bool drawError){
...
double HNLSRPlotter::GetCutFromEff(TH2D* h , int xbin, double WP){
...
Double_t HNLSRPlotter::fitf(Double_t *x,Double_t *par) {
...
void HNLSRPlotter::Draw_MVA_WP(vector<TH2D*>hists, vector<TString> histsname,  double WP,TString HistName, TString dirName){
...
void HNLSRPlotter::SaveHistsWithRatio(vector<TH1D*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> scales){
...
void  HNLSRPlotter::DrawEfficiency( vector<TH1D*> hists, vector<TString> legNames, TString HistName, TString dirName){
...
void HNLSRPlotter::draw_hist_canvas(TH1D *hist_default,  TString HistName){
