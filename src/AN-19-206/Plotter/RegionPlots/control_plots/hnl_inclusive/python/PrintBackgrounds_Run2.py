...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'process.log'),  # Log to a file                                                                                                                                                                                      
...
def GetListFromFile(listpath):
    if not os.path.exists(listpath):
        logging.info(f"File {listpath} MISSING")
        exit()
...
def GetHistError(HistName, InDir, Uncert):
    ...
    with ROOT.TFile(InDir) as f:
        ...
        if not hist:
            logging.info(f"Hist does not exist: {InDir} {HistName}")
            return 0
    ...
    try:
        ...
        with ROOT.TFile(InDir) as f:
            ...
            if hist:
                ...
                logging.info(f"Histogram {HistName} not found in {InDir}")
                return 0
    except Exception as e:
        logging.info(f"Error opening file {InDir}: {e}")
        return 0
...
def main():
    ...
    if args.Era == 'NULL' or args.Flag == 'NULL':
        logging.info("Error: Missing required arguments.")
        return
    ...
    paths = {
        ...
        "WG": os.path.join(base_path, "HNL_ControlRegion_Plotter_SkimTree_HNMultiLepBDT_WG.root"),
        "ZG": os.path.join(base_path, "HNL_ControlRegion_Plotter_SkimTree_HNMultiLepBDT_ZG.root"),
        "Other_Conv": os.path.join(base_path, "HNL_ControlRegion_Plotter_SkimTree_HNMultiLepBDT_Other_Conv.root"),
    ...
    WGInt = integrals["WG"]
    ZGInt = integrals["ZG"]
    OtherConvInt = integrals["Other_Conv"]
    WZInt = integrals["WZ"]
    WWInt = integrals["WW"]
    ...
    BkgInt = NonPromptInt + CFInt + WZInt + qqZZInt + ggZZInt + WWInt + TopInt + OtherInt + WGInt+ZGInt+OtherConvInt
    TotalPrompt = WZInt + qqZZInt + ggZZInt + WWInt + TopInt + OtherInt
    TotalConv = WGInt+ZGInt+OtherConvInt
    
    # Logging.Info or use the results as needed
    logging.info(f"Total Prompt Integral: {TotalPrompt}")
    logging.info(f"Total Conv Integral: {TotalConv}")
    logging.info(f"Total Fake Integral: {NonPromptInt}")
    
    logging.info(f"Total Background Integral: {BkgInt}")
    ...
    total_err_conv = calculate_total_error( [errors["WG"],   errors["ZG"],  errors["Other_Conv"]])
    bkg_err = calculate_total_error([errors["NonPrompt"],  total_err_conv, errors["CF"], total_err_prompt])

    # Calculate significance
    significance = (DataInt - BkgInt) / math.sqrt(errors["Data"]**2 + bkg_err**2)

    # Logging.Info or use the results as needed
    logging.info(f"Significance: {significance}")
    ...
    # Logging.Info the final output path for debugging
    logging.info(("@@@" * 50))
    logging.info(f"outfile_path = {outfile_path}")
    ...
    with open(outfile_path, "w") as f:
        ...
        write_formatted_line(f, "NBkg[Conv]", TotalConv, total_err_conv)
    ...
    # Reopen the file and logging.info its contents to screen
    with open(outfile_path, "r") as f:
        content = f.read()
        logging.info(content)  # Print the content to screen
