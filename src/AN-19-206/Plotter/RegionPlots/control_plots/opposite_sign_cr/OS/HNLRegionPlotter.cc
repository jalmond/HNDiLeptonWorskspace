...
void HNLRegionPlotter::AddMisc(TString CR){
  ...
  AddHist("MainPlots/Lep1_pt" ,"Lep Pt1"  , "", {0, 20.,25.,30.,50.,100.,  150.,  200.,350},20 ,350);
  AddHist("MainPlots/Lep2_pt" ,"Lep Pt1"  , "", {10., 15,20,30,50., 100.,  150.,  200.,350},20 ,350);
  AddHist("Leptons/Lep_1_pt" ,"P^{1}_{T} GeV" , "GeV", {0, 20.,40., 50.,75.,100.,150., 200}, 0, 200);
  AddHist("Leptons/Lep_2_pt" ,"P^{2}_{T} GeV" , "GeV", {0.,10.,15., 25.,30,35,40,45,50,75,100.,200}, 0, 200);
  AddHist("Standard/Lepton_pt" ,"Lep Pt"  , "", {1},0, 1000);
  AddHist("Standard/M_ll"  ,"M_{ll}"  , "GeV",    {1},50,500);
  AddHist("Standard/Lepton_eta" ,"#eta Lepton" , "int", {1},-2.5,2.5);
...
void HNLRegionPlotter::AddLepton(int nlep,TString CR){
  ...
  AddHist("Leptons/Lep_1_pt" ,"P^{1}_{T} GeV" , "GeV", {0, 20.,40., 50.,75.,100.,150., 200}, 0, 200);
  AddHist("Leptons/Lep_2_pt" ,"P^{2}_{T} GeV" , "GeV", {0.,10.,15., 25.,30,35,40,45,50,75,100.}, 0, 100);
...
void HNLRegionPlotter::SetupPlotter(TString FlagDir=""){
  ...
  infilepath = ENV_MERGEDFILE_PATH + "/"+AnalyserName+"/OS_Control/"+ DateFileTag + "/"+FlagDir+"/"+Era;
...
void HNLRegionPlotter::draw_canvas(THStack *mc_stack, TH1D *mc_staterror, TH1D *mc_allerror, TH1D *hist_data, vector<TH1D *> hist_signal, TLegend *legend, bool DrawData, TFile *outputf){
  ...
  c1->SaveAs(thiscut_plotpath+"/"+HistNames_NoDir+".png");

  system("cp ~/index.php " + thiscut_plotpath+"/" );
...
void HNLRegionPlotter::SetupSampleInfo(){
  ...
  map_sample_string_to_list["DYJetsMerged"] = {"DYJetsMerged"};//,"DYJets10to50_MG"};
  map_sample_string_to_list["DYNNLO"] = {"DYJetsToMuMu_MiNNLO"};//,"DYJets10to50_MG"};
  map_sample_string_to_list["DYTauTau"] = {"DYJetsToTauTau_MiNNLO"};//,"DYJets10to50_MG"};
  ...
  map_sample_string_to_list["ttbar"] = {"TTLL_powheg","SingleTop_tW_top_NoFullyHad","SingleTop_tW_antitop_NoFullyHad"};
  ...
  map_sample_string_to_legendinfo["DYNNLO"] = make_pair("DY", kYellow);
  map_sample_string_to_legendinfo["DYJetsMerged"] = make_pair("DY", kYellow);
  map_sample_string_to_legendinfo["DYTauTau"] = make_pair("DYTauTau", kOrange);
...
void HNLRegionPlotter::LeptonChannel(TString ch){
  ...
  for(int i=0; i < HistPath.size() ; i++){
    if(ch=="OSMuMu") LeptonChannels.push_back(11);
