...
FilePath="/data6/Users/jalmond/2020/Plotter/HNDiLeptonWorskspace/InputFiles/MergedFiles/HNL_ControlRegion_Plotter/OS_Control/"+DateDir+"/"

DataPath = FilePath + args.Flag + "/"+ args.Era + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_data.root"
NonPromptPath = FilePath + args.Flag + "/"+ args.Era + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_NonPrompt.root"
CFPath = FilePath + args.Flag + "/"+ args.Era + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_CF.root"
WZPath = FilePath + args.Flag + "/"+ args.Era + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_WZ.root"
WWPath = FilePath + args.Flag + "/"+ args.Era + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_WW.root"
ZZPath = FilePath + args.Flag + "/"+ args.Era + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_ZZ.root"
TopPath = FilePath + args.Flag + "/"+ args.Era + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_Top.root"
OtherPath = FilePath + args.Flag + "/"+ args.Era + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_Other.root"
print("CFPath " +CFPath)
HistName = "/"+Region+"/"+args.ID+"/"+args.Channel+"/NObj/N_Mu"

SSList="BkgLists/"+args.List+".txt"
PromptList = GetListFromFile(SSList)
PromptDir = "/data6/Users/jalmond/2020/Plotter/HNDiLeptonWorskspace/InputFiles/MergedFiles/HNL_ControlRegion_Plotter/OS_Control/"+DateDir+"/"+args.Flag + "/"+ args.Era
...
TotalErrPrompt=0
for x in PromptList:
    PRVal = GetHistIntegral(HistName, PromptDir + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_"+x+".root")
    TotalPrompt =TotalPrompt+PRVal
    VPromptInt.append(PRVal)
    PRErr=GetHistError(HistName, PromptDir + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_"+x+".root",0.15)
...
ConvDir = "/data6/Users/jalmond/SKFlatOutput/Run2UltraLegacy_v3/HNL_ControlRegion_Plotter/"+args.Era+"/"+args.Flag+"__RunConv__/"
VConvInt = []
VConvErr = []
TotalConv =0
TotalErrConv=0
for x in ConvList:
    PRVal = GetHistIntegral(HistName, ConvDir + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_"+x+".root")
    TotalConv =TotalConv+PRVal
    VConvInt.append(PRVal)
    PRErr=GetHistError(HistName, ConvDir + "/HNL_ControlRegion_Plotter_SkimTree_DileptonBDT_"+x+".root",0.15)
...
WZErr  = GetHistError(HistName, WZPath,    0.15)
ZZErr  = GetHistError(HistName, WZPath,    0.15)
WWErr  = GetHistError(HistName, WZPath,    0.20)
TopErr  = GetHistError(HistName, WZPath,    0.20)
OtherErr  = GetHistError(HistName, WZPath,    0.3)
...
for x in range(0,len(VPromptInt)): 
    ...
    Print_BkgErr = float("{:.2f}".format(VPromptErr[x]))
    f.write("Bkg : "+ PromptList[x] + " "*(50-len(PromptList[x] )) + str(Print_BkgInt) + " " + " \pm " + str(Print_BkgErr)+ '\n')
f.write("--"*50 +"\n")
...
print "="*50
print Region+"/"+args.ID+"/"+args.Channel + " Significance =  " + str(Signif)
print "="*50
print "-"*50
