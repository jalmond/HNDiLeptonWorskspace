...
PromptDir = "/data6/Users/jalmond/SKFlatOutput/Run2UltraLegacy_v3/HNL_ControlRegion_Plotter/"+args.Era+"/"+args.Flag+"__RunPrompt__/"
VPromptInt = []
VPromptErr = []
TotalPrompt =0
TotalErrPrompt=0
for x in PromptList:
    PRVal = GetHistIntegral(HistName, PromptDir + "/HNL_ControlRegion_Plotter_SkimTree_HNMultiLepBDT_"+x+".root")
    TotalPrompt =TotalPrompt+PRVal
    VPromptInt.append(PRVal)
    PRErr=GetHistError(HistName, PromptDir + "/HNL_ControlRegion_Plotter_SkimTree_HNMultiLepBDT_"+x+".root",0.15)
...
TotalErrConv=0
for x in ConvList:
    PRVal = GetHistIntegral(HistName, ConvDir + "/HNL_ControlRegion_Plotter_SkimTree_HNMultiLepBDT_"+x+".root")
    TotalConv =TotalConv+PRVal
    VConvInt.append(PRVal)
    PRErr=GetHistError(HistName, ConvDir + "/HNL_ControlRegion_Plotter_SkimTree_HNMultiLepBDT_"+x+".root",0.15)
