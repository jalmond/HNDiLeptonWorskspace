...
void SSControlRegion_Limit(){

  TString DATE = "Oct11";
  vector<TString> Eras ;
  Eras =  {"Run2"};

  vector<TString> TriLepReg ={"LimitShape_WZ_SR1"};
  ...
  SSControlRegionJob(DATE,3, Eras, "LLL",{"WZ_SS","ZZ_SS","WW_SS","Top_SS","Other_SS","NonPrompt","Conv"} , TriLepReg , "HNL_Limit_ControlRegion_SSCRPlots", "List3","LLL");
...
void RunFunction(TString DateFileTag,int nlep,vector<TString> eras,vector<TString> channels, TString PlotterTag, TString Flag,vector<TString> Bkgs, TString Region, TString InputList){
  ...
  for (auto year : eras){
	...
	for (auto ID : Dirs) system("python PrintBackgrounds_AN.py -f "+Flag+" -e "+year+" -r "+Region+" -i " + ID + "  -c "+channel+" -d "+ DateFileTag  + " -a HNL_ControlRegion_Plotter -p "+PlotterTag + " -l " + InputList );
      ...
      ///// HISTs Setup
      AddHist("LimitExtraction/MuMuMu/"+Dirs[0]+"/Binned","","",{1},0,10000);
k
