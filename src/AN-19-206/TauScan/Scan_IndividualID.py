...
for file_path in final_files:
    ...
    try:
        ...
        for dir_name in channel_cutflow_dir.GetListOfKeys():
            ...
            if not "JetT" in dir_name_str:
                continue

            if not "HNL_ULID" in dir_name_str:
                continue
            ...
            bin1_sum = 0 if default_histogram.GetBinContent(1) == 0 else histogram.GetBinContent(1) / default_histogram.GetBinContent(1)
            bin2_sum = 0 if default_histogram.GetBinContent(2) == 0 else histogram.GetBinContent(2) / default_histogram.GetBinContent(2)
            bin3_sum = 0 if default_histogram.GetBinContent(3) == 0 else histogram.GetBinContent(3) / default_histogram.GetBinContent(3)
        ...
        hist_bin1.GetYaxis().SetRangeUser(0.9,1.05);
        ...
        #print(f"Saving  TauPlots/{file_path.split('/')[-1].replace('.root', '_comparison.png')}")
        #canvas.SaveAs(f"TauPlots/{file_path.split('/')[-1].replace('.root', '_comparison.png')}")
