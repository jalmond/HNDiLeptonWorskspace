...
void FakeCut_ip3d(TString sample = "ZGTo2LG",TString channel="muon", TString var="ip3d",TString type="-1.000000", TString year="2016"){
  ...
  double eff = data_hist->Integral(data_hist->FindBin(-3.), data_hist->FindBin(3.))/ data_hist->Integral();
  if ( channel.Contains("el") )eff = data_hist->Integral(data_hist->FindBin(-5.), data_hist->FindBin(5.))/ data_hist->Integral();


  double effL = data_hist->Integral(data_hist->FindBin(-10.), data_hist->FindBin(10.))/ data_hist->Integral();
  if ( channel.Contains("el") )effL = data_hist->Integral(data_hist->FindBin(-10.), data_hist->FindBin(10.))/ data_hist->Integral();
  ...
  TLine *devz;
  if ( channel.Contains("el") )devz = new TLine(-5.,0.1,-5.,data_hist->GetMaximum());
  else devz = new TLine(-3.,0.1,-3.,data_hist->GetMaximum());

  devz->SetLineWidth(2);
  devz->SetLineStyle(1);
  devz->Draw("SAME");
  TLine *devz2;
  if ( channel.Contains("el") )devz2 = new TLine(5.0,0.1,5.,data_hist->GetMaximum());
  else  devz2 = new TLine(3.0,0.1,3.,data_hist->GetMaximum());
  ...
  DrawLatexWithLabel(year,_leptype,0.25,0.78);
  DrawLatexWithLabel(year,"Cut Eff. =["+TString(s_eff)+","+TString(s_effL)+"]",0.25,0.83);
