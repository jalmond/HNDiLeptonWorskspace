...
void IDMVAPlotterConvRun(TString era, vector<TString>  EtaBins,vector<TString> PtBins,vector<TString> MVAStrings,TString InDir, TString WebSite_Label);
void IDMVAPlotterConvRun2D(TString era, vector<TString>  EtaBins,vector<TString> PtBins,vector<TString> MVAStrings,TString InDir, TString InDirDef, TString WebSite_Label);

void IDMVAPlotterConv(){


  vector<TString> Eras = {"2017","2018"};                                                                                                                        

  vector<TString> MVAStrings = {"MVA__El_ED_Conv_v5"};


  vector<TString> PtBins = {"Ptgt100_", "Ptlt100_", "Ptlt20_","Ptlt50_","Ptlt15_","Ptlt80_"};

  TString LabelForWeb = "MVAScoreConv_2023_Sep15v1";

  vector<TString> Regions = {"BB","EC"};
  for(auto ir :Regions){
    //for(auto Era: Eras) IDMVAPlotterConvRun(Era ,{ir} , {""}, MVAStrings, "BDTVariable", LabelForWeb+"_EtaBins");

    for (auto Era : Eras) IDMVAPlotterConvRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesCF"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterConvRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesCF"+ir+"2", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterConvRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesNPCF"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterConvRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesNPCF"+ir+"2", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterConvRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesNPCF"+ir+"3", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterConvRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesNP"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
  ...
  for(auto Era : Eras) IDMVAPlotterConvRun(Era ,{"BB"} ,   {""}, MVAStrings, "BDTVariables", LabelForWeb+"_EtaBins");
  for(auto Era : Eras) IDMVAPlotterConvRun(Era ,{"EC"} ,   {""}, MVAStrings, "BDTVariables", LabelForWeb+"_EtaBins");
  //////////
  for(auto Era : Eras)IDMVAPlotterConvRun(Era ,{"BB","EC"} , {""}, MVAStrings, "SSBDTVariables",LabelForWeb+"_SS");
  //////////
  for(auto Era : Eras)IDMVAPlotterConvRun2D(Era ,{"BB"} ,      {""}, MVAStrings,    "BDTVariablesNPCFBB", "BDTVariables", LabelForWeb+"_2D");
  for(auto Era : Eras)IDMVAPlotterConvRun2D(Era ,{"EC"} ,      {""}, MVAStrings,    "BDTVariablesNPCFEC", "BDTVariables", LabelForWeb+"_2D");
...
void IDMVAPlotterConvRun(TString era, vector<TString>  EtaBins,vector<TString> PtBins,vector<TString> MVAStrings,TString InDir, TString WebSite_Label){
  ...
  for (auto year : eras){
	...
	for (auto MVAString : MVAStrings){
	  ...
	  Plotter.SvsB_Comppath(path+"/HNL_LeptonIDBDTStudies_Conv.root");
	  Plotter.SvsB_PromptHistPrefix( "TightID/Electron/IsPrompt"+Binlabel+"_/MVA__");
	  Plotter.SvsB_CompHistPrefix("TightID/Electron/IsConv"+Binlabel+"_/MVA__");
	  ...
	  double AUC = Plotter.draw_SvsB_canvas({hist_Eff,hist_Eff_Conv},  IDs, Legends , MVAString+"_"+InDir+"_"+Binlabel,WebSite_Label);
