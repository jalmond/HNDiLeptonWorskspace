...
void IDMVAPlotterCF(){
  ...
  vector<TString> MVAStrings = {"MVA__El_ED_CF_v5"};


  vector<TString> PtBins = {"Ptgt100_", "Ptlt100_", "Ptlt20_","Ptlt50_","Ptlt15_","Ptlt80_"};

  TString LabelForWeb = "MVAScoreCF_2023_Sep15v1";

  vector<TString> Regions = {"BB","EC"};
  for(auto ir :Regions){
    //for(auto Era: Eras) IDMVAPlotterCFRun(Era ,{ir} , {""}, MVAStrings, "BDTVariable", LabelForWeb+"_EtaBins");
    
    
    for (auto Era : Eras) IDMVAPlotterCFRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesNP"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterCFRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConvNP"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterCFRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConvNP"+ir+"2", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterCFRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConvNP"+ir+"3", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterCFRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConv"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterCFRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConv"+ir+"2", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterCFRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConv"+ir+"3", "BDTVariable", LabelForWeb+"_PtBins2D");
...
void IDMVAPlotterCFRun2D(TString era, vector<TString>  EtaBins,vector<TString> PtBins,vector<TString> MVAStrings,TString InDir, TString InDirDef,TString WebSite_Label){
  ...
  for (auto year : eras){
	...
	map<TString, double> AUCValues;
	  ...
	  cout << path+"/HNL_LeptonIDBDTStudies_Signal.root" << endl;
          vector<TH1D*> vHists;
