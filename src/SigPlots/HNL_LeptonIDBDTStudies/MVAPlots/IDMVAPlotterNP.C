...
void IDMVAPlotterNP(){
  ...
  vector<TString> MVAStrings = {"MVA__El_ED_Fake_v5"};


  vector<TString> PtBins = {"Ptgt100_", "Ptlt100_", "Ptlt20_","Ptlt50_","Ptlt15_","Ptlt80_"}; 

  TString LabelForWeb = "MVAScoreNP_2023_Sep15v1";
  
  vector<TString> Regions = {"BB","EC"};
  for(auto ir : Regions){
    //    for(auto Era: Eras) IDMVAPlotterNPRun(Era ,{ir} , {""}, MVAStrings, "BDTVariable", LabelForWeb+"_EtaBins");

    for (auto Era : Eras) IDMVAPlotterNPRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesCF"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterNPRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesCF"+ir+"2", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterNPRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConvCF"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterNPRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConvCF"+ir+"2", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterNPRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConvCF"+ir+"3", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterNPRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConv"+ir+"1", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterNPRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConv"+ir+"2", "BDTVariable", LabelForWeb+"_PtBins2D");
    for (auto Era : Eras) IDMVAPlotterNPRun2D(Era ,{ir} ,   PtBins, MVAStrings,"BDTVariablesConv"+ir+"3", "BDTVariable", LabelForWeb+"_PtBins2D");
  }
...
void IDMVAPlotterNPRun2D(TString era, vector<TString>  EtaBins,vector<TString> PtBins,vector<TString> MVAStrings,TString InDir, TString InDirDef,TString WebSite_Label){
  ...
  for (auto year : eras){
    for (auto EtaBin : EtaBins){
	  ...
	  cout << InDirDef + "/Electron/IsPrompt"+Binlabel+"_/"+MVAString << endl;
	  ...
	  cout << hist_prompt << " " << hist_prompt_def << endl;
