...
void SignalRegionPlotSR1(){
  ...
  for (auto year : eras){
	...
	TString sr_hist= "LimitSR1/"+channel+"/HNL_UL/N1Mass_Central";
	...
	if(channel != "MuMu")hn_cf->SetFillColor(74);
	hn_conv->SetFillColor(kSpring-7);
	hn_fake->SetFillColor(870);
	
	hs->Add(hn_prompt);
	hs->Add(hn_fake);
	if(channel != "MuMu")hs->Add(hn_cf);
	hs->Add(hn_conv);
...
;
	...
	DrawLatexWithLabel(year,SScale,0.25,0.58);
	...
	if(channel != "MuMu")legend->AddEntry(hn_cf,"CF","f");
	legend->AddEntry(hn_conv,"Conv","f");
	legend->Draw();
	
	
	TString save_pdf= output + "/SR1"+channel+"_"+year+"_"+mass+".pdf";
	TString save_png= output + "/SR1"+channel+"_"+year+"_"+mass+".png";
	
	c1->SetLogy();
	c1->SaveAs(save_pdf);
	c1->SaveAs(save_png);
	
	system("ssh jalmond@lxplus.cern.ch 'mkdir -p /afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalRegion1LimitInput/'");
      
	system("ssh jalmond@lxplus.cern.ch 'cp  /afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/SignalSplit/index.php /afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalRegion1LimitInput/'");
	
	system("scp " +save_pdf+" jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalRegion1LimitInput/");
	system("scp " +save_png + " jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalRegion1LimitInput/");
