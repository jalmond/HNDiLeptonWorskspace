...
void GetSignalEfficiency_combinedSR_EE(TString analysername="HNtypeI_Dilepton" ,TString _chan = "Schannel",bool setlog_axis=false, TString x_range="all"){ 
  ...
  vector <TString> channel ={"EE"};

  vector<TString> elIDs = {"passTightID","HNTight2016","passTightID_nocc","HNTightV1","HNTightV2","HNTightV3","HNMediumV1","HNMediumV2","HNMediumV3","passMediumID"};
  ...
  for(unsigned int j = 0 ; j < SR.size(); ++j){
	...
	AllLegendEntry(legend_g,_vgraphs[ig],_sr+":"+IDs[ig],"lp");
      ...
      TString save_sg= output + "/El_hist_"+_sr+"highmass_CombinedSR1SR2_"+analysername+"_"+_chan+"_"+_channel+".pdf";
