...
void GetSignalEfficiency_combinedSR_MM(TString analysername="HNtypeI_Dilepton" ,TString _chan = "Schannel",bool setlog_axis=false, TString x_range="all"){ 
  ...
  vector<TString> muIDs  = { "HNTight2016","POGTightPFIsoTight", "POGHighPtMixTight","POGHighPtTight"};
  ...
  for(unsigned int j = 0 ; j < SR.size(); ++j){
	...
	AllLegendEntry(legend_g,_vgraphs[ig],_sr+":"+IDs[ig],"lp");
      ...
      TString save_sg= output + "/Mu_graph_"+_sr+"highmass_CombinedSR1SR2_"+analysername+"_"+_chan+"_"+_channel+".pdf";
