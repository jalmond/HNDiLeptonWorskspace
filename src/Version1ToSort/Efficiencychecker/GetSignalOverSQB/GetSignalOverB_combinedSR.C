...
void GetSignalOverB_combinedSR(TString _y = "",TString suffix="",TString channel="EE"){ 
  ...
  while ( (key1 = (TKey*)next()) ) {
    ...
    //PreselEEYields/HNDilepton_EE_HN_mva0.900000_mva0.900000_mva0.720000_LowPt_SR2_events
    hname = hname.ReplaceAll("_SR2_events","");
    hname = hname.ReplaceAll("HNDilepton_EE_","");
  ...
  for (auto year : years){
	...
	for(auto _id : IDs){
	  ...
	  //presel_EE_same_sign/presel_EE_same_sign_SR1_njets_HNDilepton_EE_HNDilepton_EE_HNTight_miso_mva0.700000_mva0.700000_mva0.500000_SR2_events_HighPt
	...
	for (auto im : masses){
	  ...
	  for(auto _id : IDs){
	    ...
	    if(im != "100") {
	      TH1D* hpass2 = (TH1D*)(filemmT->Get(n_sr_hist));
	      if(hpass1&&hpass2)hpass1->Add(hpass2);
	    }
