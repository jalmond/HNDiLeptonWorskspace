...
void GetSignalOverB_combinedSR_ID_SSWW_MVA(){
...
void GetSignalOverB_combinedSR_IDMAIN(vector<TString> IDs, TString _y = "2016",TString isotag="_iso_",TString suffix="ipB",TString smass, double mass, TString channel="EE"){ 
  ...
  for (auto y : years)  for (auto im : masses) {
      TString sigpathS = ENV_MERGEDFILE_PATH+ "/"+analysername+"/"+y+"/SIG/"+analysername+"_SSWWTypeI_NLO_SF_M"+im+".root";
  ...
  for (auto year : years){
	...
	for (auto im : masses){
	  ...
	  ///-- Get signal file
	  TString sigpathS = ENV_MERGEDFILE_PATH+ "/"+analysername+"/"+year+"/SIG/"+analysername+"_SSWWTypeI_NLO_SF_M"+im+".root";
