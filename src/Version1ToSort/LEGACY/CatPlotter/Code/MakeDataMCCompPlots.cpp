...
int main(int argc, char *argv[]) {
  ...
  system(("scp -r " + output_path + " jalmond@lxplus022.cern.ch:~/www/SNU/SKAnalyzer/").c_str());
...
int MakePlots(string hist) {
  ...
  while(!histo_name_file.eof()) {
    ...
    for(unsigned int ncut=0; ncut<allcuts.size();  ncut++){
      string name = allcuts.at(ncut) + "/" +allcuts.at(ncut) + "_"+ h_name+ "_HNtypeI_JA_" + id+ allcuts.at(ncut);
      bool isSS(false);
