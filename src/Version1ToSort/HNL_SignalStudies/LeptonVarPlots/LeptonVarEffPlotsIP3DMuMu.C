...
void LeptonVarEffPlotsIP3DMuMu(){
  ...
  for (auto year : eras){
	...
	for (auto var : vars){
	  ...
	  for(int ibin = 0; ibin < 4;  ibin++){
	    ...
	    for( int j1=0; j1 < 71; j1++){
	      double IP_1 = double(3.+0.1*double(j1));
	      std::string IP1= std::to_string(IP_1);

	      
	      TString sr_hist=  "OptMuMu/Sig"+var+"_"+TString(IP1);
	      TString sr_Fakehist=  "OptMuMu/Fake"+var+"_"+TString(IP1);
	      TString sr_Convhist=  "OptMuMu/Conv"+var+"_"+TString(IP1);
