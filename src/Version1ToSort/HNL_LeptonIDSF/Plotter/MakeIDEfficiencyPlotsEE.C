...
void MakeIDEfficiencyPlotsEE(){
  ...
  vector<TString> eras =  {"2017","2018"};  //{"2016preVFP","2016postVFP"};//"2016preVFP","2016postVFP","2017","2018"};
  ...
  for (auto era : eras){
    ...
    vector<TString> IDs = {"CFL","CFM","CFT","CFL_ED","CFM_ED","CFT_ED"};
    vector<TString> ID2s = {"_BDTGv2_MD3","_BDTGv2Pt_MD3","_BDTGv3_MD3"}; 
	...
	for (auto ID : IDs){
	  for (auto ID2 : ID2s){
	    ...
	    TString tag = dataset+"_"+era+"_"+EtaBin+"_"+ID+ID2;
	    ...
	    TString denom = "EE/Pt_"+EtaBin+"_HNL_ULID_"+ID+ID2+"_denom";
	    TString num   = "EE/Pt_"+EtaBin+"_HNL_ULID_"+ID+ID2+"_num";
	    ...
	    TString outpath = output + "/IDSF_"+era+"_"+EtaBin+"_"+dataset+"_"+ID+ID2;
	    ...
	    SaveAndCopyLXPLUS(c1,outpath,analysername,"IDSF_Electron_BDT_Version2");
	  }
	}
      }
    }
