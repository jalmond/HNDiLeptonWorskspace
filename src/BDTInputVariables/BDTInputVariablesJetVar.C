...
void BDTInputVariablesJetVar(TString era="2017",TString histDir="TopJets_TightMu/", TString histpath="CJ_NonPromptLep_JetDisc_DeepJet_gt35_Electron_TightMuE", TString label ="p_{T} > 35 GeV", double xmin=0, double xmax=1,double _rebin=10,TString xtitle=""){
  ...
  for(auto  mHistName:  HistNames){
    ...
    for (auto iLog : LogOpt){
      ...
      TString FullHistNameP=FullHistName;
      FullHistNameP=FullHistNameP.ReplaceAll("nJet_","nPromptLep_Jet_");
      ...
      TH1D* Hist_MC_Full = (TH1D*)(File_MCsample->Get(FullHistName))->Clone(FullHistName+"MCP");

      // Make NP
      Hist_MC_Full->Add(Hist_MC,-1);

      Hist_DATA->Add(Hist_MC_Full,-1);
      ...
      SaveAndCopyLXPLUS(c1,outpath,analysername,"BDTvariables_JetVar2");
