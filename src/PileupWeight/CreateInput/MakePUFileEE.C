...
void MakePUFileEE(TString year="2016",TString dataset="DoubleEG", TString dataset2="Muon"){
  ...
  TString pathData2= "/Users/john/HNDiLeptonWorskspace/OutputTool/MergedFiles/HNtypeI_JA/"+year+"/HNtypeI_JA_SkimTree_SSNonIso_"+dataset2+".root";
  ...
  TFile * fdata2 = new TFile(pathData2);
  
  /// Set Plotting style
  setTDRStyle();
  gStyle->SetPalette(1);
    
  TString outfile = "PileupRate13TeV_"+year+".root";
  ...
  for(unsigned int i=0; i < fakes40.size(); i++){
    ...
    TString denom = "Z_EE_/Z_EE__nPV_HNtypeI_JA_EE_passMediumID";

    TString denom2 = "Z_MuMu_/Z_MuMu__nPV_HNtypeI_JA_MuMu_POGTightPFIsoLoose";
    
    //    return;
    TH1* h_nV_mc   = (TH1*)fmc->Get(denom.Data());
    TH1* h_nV_data = (TH1*)fdata->Get(denom.Data());

    TH1* h_nV_mc2   = (TH1*)fmc->Get(denom2.Data());
    TH1* h_nV_data2 = (TH1*)fdata2->Get(denom2.Data());


    CheckHist(h_nV_mc);
    CheckHist(h_nV_data);
    CheckHist(h_nV_mc2);
    CheckHist(h_nV_data2);
    
    TString name = "PUReweight" ;
    
    TH1* _h_nV_mc = (TH1*)h_nV_mc->Clone((name+"MC").Data());
    TH1* _h_nV_data = (TH1*)h_nV_data->Clone(name );

    TH1* _h_nV_mc2 = (TH1*)h_nV_mc2->Clone((name+"MC2").Data());
    TH1* _h_nV_data2 = (TH1*)h_nV_data2->Clone(name+"2" );

    _h_nV_mc->Add(_h_nV_mc2);
    _h_nV_data->Add(_h_nV_data2);
