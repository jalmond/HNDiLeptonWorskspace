...
void GetSignalOverB_combinedSR_ID_MVA(){

  vector<TString> years={"2017"};//,"2017","2018"};
  
  for (auto y : years) {
    vector <TString> masses = {"1500"};//,"500","1000"};                                                                                                                                    
    vector <double> d_masses = {1500.};//,500.,1000.};                                                                                                                                       

    vector<TString> IDnames = GetIDList();

    for(int i = 0; i <  masses.size(); i++){
      GetSignalOverB_combinedSR_IDMAIN(IDnames,y,"", "FullOpt_MVA", masses[i], d_masses[i]);
...
std::vector<TString> GetIDList(){
  ...
  TString promptpath1 = ENV_MERGEDFILE_PATH+ analysername + "/DEF/"+analysername+"_SkimTree_HNMultiLep_DoubleMuon.root";
  cout << promptpath1 << endl;
  vector<TString> IDs;
    
  TFile * _file1 = new TFile(promptpath1);
  TH1* h_id  = (TH1*)_file1->Get("FillEventCutflow/Fullopt_testhist_events_FillEventCutflow");
...
void GetSignalOverB_combinedSR_IDMAIN(vector<TString> IDs, TString _y = "2016",TString isotag="_iso_",TString suffix="ipB",TString smass, double mass, TString channel="EE"){ 
  ...
  bool debug(true);
  // basic declarations 
  TString ptbin="";  TString _chan = "Schannel"; TString analysername="HNL_IDOpt";
  ...
  vector<TString> code_names= {"HNL_IDOpt"};
  ...
  for (auto y : years)  for (auto im : masses) {
      TString sigpathS = ENV_MERGEDFILE_PATH+ "/"+analysername+"/"+y+"/SIG/"+analysername+"_DYTypeI_NLO_SF_M"+im+".root";
  ...
  for (auto year : years){
    ...
    for(auto __sr : SR){
      ...
      TString _sr=__sr.first;
      vector<TString> PTBins = __sr.second;
	...
	//#if(ptbin == "_LowPt" && _sr == "SR1") continue;
	
	//-- vector of IDvalues
	vector<double> IDvalues;	

	//-- Make string for data file
	TString promptpath = ENV_MERGEDFILE_PATH+ analysername+"/"+year+"/"+analysername+"_SkimTree_HNMultiLep_DoubleMuon.root";
	...
	TString n_sr_hist ="PreselMuMuYields/"+suffix+ptbin+"_"+_sr+"_events";
	TString n_sr_hist_def ="PreselMuMuYields/FullOpt_StandardIDs"+ptbin+"_"+_sr+"_events";

	
	//-- access histogram in bkg file                                                                                                                                    
	cout << n_sr_hist << " " << n_sr_hist_def << endl;
	cout << promptpath << " " <<file_prompt << endl;
	TH1D* hpass = (TH1D*)(file_prompt->Get(n_sr_hist));
	TH1D* hpass2 = (TH1D*)(file_prompt->Get(n_sr_hist_def));
	cout << hpass << " " << hpass2 << endl;
	if(!hpass) continue;
	
	if(hpass2)hpass->Add(hpass2);
	...
	for (auto im : masses){
	  ...
	  ///-- Get signal file
	  TString sigpathS = ENV_MERGEDFILE_PATH+ "/"+analysername+"/"+year+"/SIG/"+analysername+"_DYTypeI_NLO_SF_M"+im+".root";
	  TString sigpathT = ENV_MERGEDFILE_PATH+ "/"+analysername+"/"+year+"/SIG/"+analysername+"_VBFTypeI_NLO_SF_M"+im+".root";
	  TString sigpathWW = ENV_MERGEDFILE_PATH+ "/"+analysername+"/"+year+"/SIG/"+analysername+"_SSWWTypeI_NLO_SF__M"+im+".root";
	  TFile * filemm = new TFile((sigpathS).Data());
	  TFile * filemmT = new TFile((sigpathT).Data());
	  TFile * filemmWW = new TFile((sigpathWW).Data());
	  ...
	  if(im != "100") {
	    ...
	    TH1D* hsigWW = (TH1D*)(filemmWW->Get(n_sr_hist));
	    TH1D* hsigWW2 = (TH1D*)(filemmWW->Get(n_sr_hist_def));
            hsigS->Add(hsigWW2);
  ...
  cout << "map_punzi size = " << map_punzi.size() << endl;
