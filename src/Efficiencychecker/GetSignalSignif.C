...
void GetSignalSignif(TString _chan = "Schannel"){ 
  ...
  if (_chan=="Schannel") ignore_masses = {"125","250","300","500","600"};
  ...
  for(unsigned int j = 0 ; j < SR.size(); ++j){
	...
	for(unsigned int l = 0 ; l < IDs.size(); l++){
	  ...
	  total_s += GetIntegral(sigpath, n_sr_hist)/ GetIntegral(sigpath , "CutFlow/NoCut_HNtypeI_JA_"+_channel + "_"+ _id);//GetSignalIntegral(d_masses[i], Years[y], sigpath, n_sr_hist, "CutFlow/NoCut_HNtypeI_JA_"+_channel + "_"+ _id);
	  ...
	  total_b += GetIntegral(promptpath, n_sr_hist);
	  total_b += GetIntegral(fakepath, n_sr_hist);
	  total_b += GetIntegral(cfpath, n_sr_hist);
	  cout  << "Year = " << Years[y] <<  " Channel " << _chan << " : "  << _channel << " SR = " << _sr << " Charge = " << sign << "  ID " << _id << "  Mass = " << masses.at(i) <<  " s = " << total_s << " b=" << total_b <<  " s/root_b= " << total_s/sqrt(total_b)  <<endl;
	  
	  double err ;
	  _y.push_back(total_s/sqrt(total_b) );
	  _ylow.push_back(0);
          _yup.push_back(0);
	  this_hist->Fill(im, total_s/sqrt(total_b) );
	  ...
	  AllLegendEntry(legend_g,gtmp,_sr+":"+_id,"lp");
	...
	//MakeDir(ENV_PLOT_PATH+FLATVERSION+"/"+Years[y]+"/"+_sr+"/"+_channel);
	...
	TString canvasname2=_sr+"_"+_channel +"_highmass2_"+sign +"_njets_"+analysername+"_JA_"+_channel+Years[y];
	TCanvas* c2 = new TCanvas(canvasname2,canvasname2, 800,800);
	c2->cd();
	
	TGraphAsymmErrors*  old_gr = Get2016SoverSB(Years[y],_sr, _channel,d_masses);
	...
	MakeDir(ENV_PLOT_PATH+FLATVERSION+"/SoverB/"	 );
	TString save_sg=ENV_PLOT_PATH+FLATVERSION+"/SoverB/graph_highmass_"+sign +"_njets_HNtypeI_JA_"+_channel+"_"+_chan+"_"+Years[y]+_sr+".pdf";
	c2->SaveAs(save_sg);
	delete c2;
      } // year
    }
