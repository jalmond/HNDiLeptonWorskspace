...
void IDMVAPlotterConv(){
  ...
  vector<TString> MVAStrings = {"MVA__El_Conv_v1", "MVA__El_Conv_v2","MVA__El_ED_Conv_v2"};

  vector<TString> Eras = {"2016","2017","2018"};
  TString LabelForWeb = "MVAScoreConv_2023_June29";

  vector<TString> PtBins = {"Ptgt200_", "Ptlt200_", "Ptlt15_","Ptlt20_","Ptlt50_"};

  for (auto Era : Eras) IDMVAPlotterConvRun2D(Era ,{"BB"} ,   PtBins, MVAStrings, "BDTVariablesNPCFBB",  "BDTVariables",  LabelForWeb+"_PtBins");
  for (auto Era : Eras) IDMVAPlotterConvRun2D(Era ,{"EC"} ,   PtBins, MVAStrings, "BDTVariablesNPCFEC",  "BDTVariables", LabelForWeb+"_PtBins");
  
  for(auto Era : Eras)IDMVAPlotterConvRun2D(Era ,{"BB"} ,      {""}, MVAStrings,    "BDTVariablesCFBB2", "BDTVariables", LabelForWeb+"_2D");
  for(auto Era : Eras)IDMVAPlotterConvRun2D(Era ,{"EC"} ,      {""}, MVAStrings,    "BDTVariablesCFEC2", "BDTVariables", LabelForWeb+"_2D");
