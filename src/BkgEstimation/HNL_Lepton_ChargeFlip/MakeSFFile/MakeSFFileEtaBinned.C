...
void MakeSFFileConfig(TString era, TString HistDir){
  ...
  TString mc_path  = "/data6/Users/jalmond/2020/HNDiLeptonWorskspace/InputFiles/MergedFiles/Run2UltraLegacy_v3/HNL_ChargeFlip/"+era +"/"+HistDir+"/HNL_ChargeFlip_SkimTree_DileptonBDT_Rates.root";
  ...
  for(auto iEta : EtaBins){
    for(unsigned int i=0; i < IDs.size(); i++){
      ...
      TString denom = IDs[i] +"CFRate_Pt/CFrate/"+iEta+"_Denom";
      TString num   = IDs[i] +"CFRate_Pt/CFrate/"+iEta+"_Num";
  ...
  for(auto iEta : EtaBins){
    for(unsigned int i=0; i < IDs.size(); i++){
      ...
      TString denom = IDs[i] +"CFRate_Pt/CFratePt/"+iEta+"_Denom";
      TString num   = IDs[i] +"CFRate_Pt/CFratePt/"+iEta+"_Num";
