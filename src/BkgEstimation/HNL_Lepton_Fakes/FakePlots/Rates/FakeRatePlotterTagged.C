  ...
  for (auto year : Eras){
    for( auto EtaBin : EtaBins){
      ...
      TH1D *hist_Numerator     = Plotter.ConstructHist(path,
						       "Fakerates/LFGen_TightEl_"+EtaBin+"_pt");
      
      TH1D *hist_Denominator   = Plotter.ConstructHist(path,
						       "Fakerates/LFGen_LooseEl_"+EtaBin+"_pt");
      ...
      TH1D *hist_TT_Numerator     = Plotter.ConstructHist(pathTT,
                                                       "Fakerates/LFTagged_TightEl_"+EtaBin+"_pt");

      TH1D *hist_TT_Denominator   = Plotter.ConstructHist(pathTT,
                                                       "Fakerates/LFTagged_LooseEl_"+EtaBin+"_pt");
      ...
      Plotter.SaveHists(hists, {"QCD LF GenMatch","QCD LF Tagged "}, "MeasurementRegion_QCD_Tagger_"+EtaBin+"_"+year, LabelForWeb, {});
