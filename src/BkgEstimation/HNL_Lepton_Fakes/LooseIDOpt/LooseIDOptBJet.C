...
void LooseIDOptBJet(){
  ...
  for(auto Bkg : BkgType){
	...
	for(auto ShiftVal : ShiftVals){
	  ...
	  //	  cout << "MCFakeOpt/DeepJetB_"+ShiftVal + "/LF_"+EtaBin  + "_"+ID+"_Loose" << endl;
	  TH1D *hist_LF_Loose          = Plotter.ConstructHist(path,"MCFakeOpt/DeepJetB_"+ShiftVal + "/LF_"+EtaBin  + "_"+ID+"_Loose");
	  TH1D *hist_LF_Tight          = Plotter.ConstructHist(path,"MCFakeOpt/DeepJetB_"+ShiftVal + "/LF_"+EtaBin  + "_"+ID+"_Tight");
	  
	  TH1D *hist_HF_Loose          = Plotter.ConstructHist(path,"MCFakeOpt/DeepJetB_"+ShiftVal + "/HF_"+EtaBin  + "_"+ID+"_Loose");
	  TH1D *hist_HF_Tight          = Plotter.ConstructHist(path,"MCFakeOpt/DeepJetB_"+ShiftVal + "/HF_"+EtaBin  + "_"+ID+"_Tight");
