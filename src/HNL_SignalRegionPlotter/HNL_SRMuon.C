...
void HNL_SRMuon(){
  ...
  for (auto era : eras){
    ...
    TString histname="N1Mass_Central";                                                                                                    
    
    TString Histname="LimitSR1/MuMu/MVAUL_UL/"+histname;
    ...
    FormatHist(hBkg,1,-999,0,2000.,0,1, "m(l1J) GeV","Events / Bin ");
    hBkg->SetLineColor(kRed);
    hBkg->SetLineWidth(2);
    hBkg->SetFillColor(kOrange);
    hBkg->SetFillStyle(3354);
    
    FormatHist(hSig1000,1,-999,0,2000., GetHistColor(0),1, "P_{T} GeV","Events / GeV");
    ...
    hBkg->GetYaxis()->SetRangeUser(0.,10);
    hBkg->Draw("hist");
    ...
    TString label = "SR1 Muon Channel";
    ...
    SaveAndCopyLXPLUS(c1,outpath,analysername,"SR1MuMuLimit");
