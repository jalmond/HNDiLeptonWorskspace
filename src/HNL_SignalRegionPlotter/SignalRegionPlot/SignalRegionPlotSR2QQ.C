...
void SignalRegionPlotSR2QQ(){
  
  vector<TString> eras =  {"2017"};
  vector<TString> channels = {"MuMu","EE","EMu"};
  vector<TString> masses= {"300","400","500","800","1000","2000","5000","10000","20000"};
  ...
  for (auto year : eras){
	...
	TString sr_hist= "LimitSR2/"+channel+"/HNL_UL/Q_HT_LT1_Central";
	...
	if(hn_cf)hn_cf->SetFillColor(74);
	if(hn_conv)hn_conv->SetFillColor(kSpring-7);
	hn_fake->SetFillColor(870);
	
	hs->Add(hn_prompt);
	hs->Add(hn_fake);
	if(hn_cf)hs->Add(hn_cf);
	if(hn_conv)hs->Add(hn_conv);
	TCanvas* c1 = new TCanvas(canvasname,canvasname, 800,800);
	
	c1->cd();
	double scalesig = hn_bkg->Integral() / hn_sig100->Integral();
	hn_sig100->Scale(hn_bkg->Integral() / hn_sig100->Integral());
	
	hn_sig100->GetYaxis()->SetRangeUser(0.1, hn_sig100->GetMaximum()*1000);
	...
	TString binlabel=channel+" Channel";
	DrawLatexWithLabel(year,binlabel,0.25,0.68);
	
        TString SScale = "Sig Scaled by " +   TString(to_string(scalesig));
        DrawLatexWithLabel(year,SScale,0.25,0.63);
	...
	if(hn_cf)legend->AddEntry(hn_cf,"CF","f");
	if(hn_conv)	legend->AddEntry(hn_conv,"Conv","f");
	legend->Draw();
	
	
	TString save_pdf= output + "/ChargeSplit_SR2"+channel+"_"+year+"_"+mass+".pdf";
	TString save_png= output + "/ChargeSplit_SR2"+channel+"_"+year+"_"+mass+".png";
	
	c1->SetLogy();
	c1->SaveAs(save_pdf);
	c1->SaveAs(save_png);
	
	system("ssh jalmond@lxplus.cern.ch 'mkdir -p /afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalRegion2LimitInput/'");
      
	system("ssh jalmond@lxplus.cern.ch 'cp  /afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/SignalSplit/index.php /afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalRegion2LimitInput/'");
	
	system("scp " +save_pdf+" jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalRegion2LimitInput/");
	system("scp " +save_png + " jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalRegion2LimitInput/");
