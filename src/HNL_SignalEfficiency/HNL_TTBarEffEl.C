...
void HNL_TTBarEffEl(){
  ...
  for (auto era : eras){
    ...
    for(auto sample : samples){
	  ...
	  TString DirName= itype+ "_EE/";
	  
	  TString sHist_Loose    = DirName+LepType+"_pt"+EtaBin+"_MVALoose";
	  TString sHist_HNLID    = DirName+LepType+"_pt"+EtaBin+"_HNL_ULID_2017";
	  ...
	  if(itype=="Fake"){
	    Ratio_HNLV2IDR->SetMaximum(.6);
	    Ratio_HNLV2IDR->SetMinimum(0.001);
	  }
	  else{
	  ...
	  SaveAndCopyLXPLUS(c1,outpath,analysername,"TTBarElectron");
