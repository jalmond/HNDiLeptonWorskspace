...
void HNL_SignalEfficiencyTightEl(){
  ...
  for (auto era : eras){
	...
	TString LepType = "Electron";
	TString DirName="SignalRecoEE/";
	
	TString Hist_DenLoose = DirName+LepType+"_ptbinned_"+EtaBin+"_MVALoose";
	TString Hist_HNLID= DirName+LepType+"_ptbinned_"+EtaBin+"_HNL_ULID_2017";
	...
	if(1){
	  ...
	  double HNLV2Int = hNumHNLV2->Integral(0.,hNumHNLV2->GetNbinsX()+1 );
          ...
          TH1D* hHNLCompR =  GetRatioHist(hHNLIDR,hHNLV2IDR);
	  ...
	  SaveAndCopyLXPLUS(c1,outpath,analysername,"ElectronTightV2");
	}
