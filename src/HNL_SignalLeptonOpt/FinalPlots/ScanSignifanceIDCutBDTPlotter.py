...
parser.add_argument('-ID', dest='ID', type=str, default='MuMu')
...
for channel in channels:
    ...
    InPath=InputDir + "HNL_SignalLeptonOpt_SkimTree_HNMultiLepBDT_MC.root"

    f_Bkg = ROOT.TFile(InPath)
    
    IDsTOPlot = ["HNTightV2", "HNLOpt_ULHNL_Peking"]
    IDsTOPlot.append(ID)
    ...
    for mass in masses:
        ...
        IDName=HistName
        IDName.replace("LimitBins/MuonSR","")
        IDName.replace("/","")
        IDName.replace(mass,"")
        ...
        val_HNV2Arr =GetSignificanceArray(outlog,hsigDYV2,hsigVBFV2,hsigSSWWV2, h_bkg,FOM,scaleSig, EXO17028Sig):
...
hPlotter.MakeOptDHist(Masses,sample_dict2)
