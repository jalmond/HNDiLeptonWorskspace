...
masses  = ["100", "400", "500","1000","2000"]

parser = argparse.ArgumentParser(description='CR plot commands')
parser.add_argument('-f', dest='Flag', type=str, default='')
parser.add_argument('-comp', dest='Comp', type=str, default='HNLOpt_ULHNTightV2')
parser.add_argument('-e', dest='Era', type=str, default='')
parser.add_argument('-signal', dest='SignalMode', type=str, default='DYVBFSSWW')
...
for channel in channels:
    ...
    for mass in masses:
        ...
        HistNames=[]
        DirName = "LimitInput"
        HistName="/"+channel+"/"+CompID+"/LimitBins/MuonSR"
        BDTMasses = ["100","400","500"]

        if mass in BDTMasses:
            DirName = "LimitInputBDT"
            HistName="/"+channel+"/"+CompID+"/"+mass+"/LimitBins/MuonSR"
        
        IDName=HistName
        IDName=IDName.replace("LimitBins/MuonSR","")
        ...
        if not CompID == "17028":
            ...
            val_HNV2= GetSignificance(outlog,SignalMode,Era,IDName,hsigDYV2,hsigVBFV2,hsigSSWWV2,hbkgV2,FOM,scaleSig,EXO17028Sig, EXO17028NSig,EXO17028NBkg,mass)

        else:
            val_HNV2 =EXO17028Sig[0][1]
        ...
        for x in HistNames:
            ...
            print val_HNV2
            _sig= GetSignificance(outlog,SignalMode,Era,IDName,hsigDY,hsigVBF, hsigSSWW,hbkg,FOM,scaleSig,EXO17028Sig,EXO17028NSig,EXO17028NBkg,mass)/val_HNV2
            _hist = x
...
for x in range(0, len(sorted_x)):
    Print (outlog, sorted_x[x][0] + " "*(maxLength-len(sorted_x[x][0])) + str(sorted_x[x][1]/len(masses)),True)
