...
masses  = ["600" ,"700", "1000","2000"]

parser = argparse.ArgumentParser(description='CR plot commands')
parser.add_argument('-f', dest='Flag', type=str, default='Mu_SelectionOptSR1__')
parser.add_argument('-e', dest='Era', type=str, default='2017')
...
for channel in channels:
    ...
    for mass in masses:
        ...
        HistNames=[]
        DirName = "LimitInput/"+channel
        HistName="/HNLOpt_ULHNTightV2_SigR1_v1_SigR2_v1_SigR3_v1/LimitBins/MuonSR"
        BDTMasses = ["100","400"]

        if mass in BDTMasses:
            DirName = "LimitInputBDT/"+channel
            HistName="/HNLOpt_ULHNTightV2_SigR1_v1_SigR2_v1_SigR3_v1/"+mass+"/LimitBins/MuonSR"
        ...
        for x in HistNames:
            ...
            HISTSPLIt=x
            HISTSPLIt=HISTSPLIt.replace("/"," ")
            sHISTSPLIt=HISTSPLIt.split()
            print x  + " : "  + str(len(sHISTSPLIt))
            if len(sHISTSPLIt) == 4:
                if not mass in sHISTSPLIt:
                    continue
