...
void DrawSigEff(){
  ...
  DrawConfig("May12","MuMu","2016preVFP", "HNLvsHNTight");
  DrawConfig("May12","MuMu","2016postVFP", "HNLvsHNTight");
  DrawConfig("May12","MuMu","2017", "HNLvsHNTight");
  DrawConfig("May12","MuMu","2018","HNLvsHNTight");
...
void DrawConfig(TString DateTag,TString Flavour, TString era, TString filepath){
  ...
  for (auto year : eras){
    ...
    for(unsigned int i = 0 ; i < MassesDYS.size() ; i++){
      ...
      TH1D *hist_Denom       = Plotter.ConstructHist(path_mass,"ChannelCutFlow/HNTightV2/"+Flavour +"/ChannelDependant_Inclusive");

      TH1D *hist_NumPresel     = Plotter.ConstructHist(path_mass,"ChannelCutFlow/HNTightV2/"+Flavour+"/ChannelDependant_Presel");
    ...
    TGraphAsymmErrors *gPreDY2 = new TGraphAsymmErrors(nDY,MassesDYD , vEffPreDY2);
    TGraphAsymmErrors *gPreSSWW = new TGraphAsymmErrors(nVBF,massesVBFD , vEffPreVBF);
    TGraphAsymmErrors *gPreSSWW2 = new TGraphAsymmErrors(nVBF,massesVBFD , vEffPreVBF2);

    Plotter.draw_SignalEff_canvas(gPreDY,gPreDY2,gPreSSWW,gPreSSWW2,  Flavour+"_DYEff",year,"HNL_ULID_"+year);
