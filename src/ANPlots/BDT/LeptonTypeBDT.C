...
void RunBDT(HNLPlotter Plotter,TString ID, TString Era, TString DateTag, TString Method, TString Indir, TString HistString,TString LabelForOutPut,vector<double> vrebin);

void LeptonTypeBDT(){

  HNLPlotter Plotter("BDT_LeptonType");
  Plotter.DoDebug=false;
  Plotter.CopyToWebsite = false;
  
  for(auto era : Plotter.Eras("2018")) {
    TString year = (era.Contains("16")) ? "2016" : era;

    RunBDT(Plotter, "BDTVariableLowPt","Muon","Mu_HF_Fake_POG");

//HNL_TC1_ULID_"+year, era, "May10","PBS","HNL_ChargeFlip_Closure_ZMass", "Separate_BinnedShift", "Closure_ElectronCF_ZMass" ,  {50,60,70,80,85,90,95,100,110,130});
    
  }
  
}

void RunBDT(HNLPlotter Plotter,TString Dir, TString flavour, TString MVA){
  
  Plotter.SetupPlotter(Era,"","HNL_Signal_Studies");
  ///data6/Users/jalmond/SKFlatOutput//Run2UltraLegacy_v3/HNL_Lepton_ID_BDT_Studies/2018/BDTPerType__/HNL_Lepton_ID_BDT_Studies_SkimTree_DileptonBDT_TTLJ_powheg.root
  TString path= TString(std::getenv("FILE_MERGED_PATH")) + "/HNL_Lepton_ID_BDT_Studies/"+Era+"/HNL_Lepton_ID_BDT_Studies_SkimTree_DileptonBDT.root";

  TH1D *hist_mass_CF             = Plotter.ConstructHist(path,ID+"/ZMass/SS", vrebin);
  TH1D *hist_Prompt              = Plotter.ConstructHist(path,ID+"/"+Indir+"/CFRate_InvPtEtaBin2_"+HistLab+"_Central_"+ID+"/ZMass/OS_Weighted_EnergyShift", vrebin);
  cout << ID+"/"+Indir+"/CFRate_InvPtEtaBin2_"+HistLab+"_Central_"+ID+"/ZMass/OS_Weighted_EnergyShift" << endl;
  cout << ID+"/"+Indir+"Up/CFRate_InvPtEtaBin2_"+HistLab+"_PSigma_"+ID+"/ZMass/OS_Weighted_EnergyShift" << endl;
  TH1D *hist_PromptUp              = Plotter.ConstructHist(path,ID+"/"+Indir+"Up/CFRate_InvPtEtaBin2_"+HistLab+"_PSigma_"+ID+"/ZMass/OS_Weighted_EnergyShift", vrebin);
  TH1D *hist_PromptDown              = Plotter.ConstructHist(path,ID+"/"+Indir+"Down/CFRate_InvPtEtaBin2_"+HistLab+"_MSigma_"+ID+"/ZMass/OS_Weighted_EnergyShift", vrebin);

  TString StatBinUp = Indir+"/CFRate_InvPtEtaBin2_"+HistLab+"_PSigma_"+ID;
  TString StatBinDown = Indir+"/CFRate_InvPtEtaBin2_"+HistLab+"_PSigma_"+ID;
  if(Method == "PBS") StatBinUp = "Separate_BinnedStatUp/CFRate_InvPtEtaBin2_"+HistLab+"_Central_"+ID;
  if(Method == "PBS") StatBinDown = "Separate_BinnedStatDown/CFRate_InvPtEtaBin2_"+HistLab+"_Central_"+ID;
  
  cout << "Stat hist " << StatBinUp+"/ZMass/OS_Weighted_EnergyShift" << endl;
  TH1D *hist_PromptSUp              = Plotter.ConstructHist(path,ID+"/"+StatBinUp+"/ZMass/OS_Weighted_EnergyShift", vrebin);
  TH1D *hist_PromptSDown              = Plotter.ConstructHist(path,ID+"/"+StatBinDown+"/ZMass/OS_Weighted_EnergyShift", vrebin);
 
  TH1D *hist_Prompt_NoShift      = Plotter.ConstructHist(path,ID+"/NoShift/CFRate_InvPtEtaBin2_NoS_Central_"+ID+"/ZMass/OS_Weighted_EnergyShift" , vrebin);
  
  cout << hist_mass_CF << " " << hist_Prompt << " " << hist_Prompt_NoShift << " " << hist_PromptSUp <<  endl; 
  ...
  cout << "Pred [no E_{el} shift] = " << hist_Prompt_NoShift->Integral() << endl;
  
  SaveHistogram(Era,DateTag, Plotter,{hist_mass_CF,hist_Prompt,hist_Prompt_NoShift,hist_PromptUp,hist_PromptDown,hist_PromptSUp,hist_PromptSDown}, {"SS Observed", "OS*R_{CF} Predicted","OS*R_{CF} Predicted [no shift]","Pred Up","Pred Down"}, ID+"_CF_Closure_"+HistString+"_"+Method+"_"+Era, LabelForOutPut, {Method});
...
void SaveHistogram(TString Era,TString DateTag,HNLPlotter plotter,vector<TH1D*>hists, vector<TString> legname, TString HistName, TString dirName, vector<TString> Labels){
  ...
  for(int i=1; i < tmp_Cent->GetNbinsX()+1; i++){
    ...
    cout << "Bin " << i << " NewErr = " << NewErr << " CENT = " << NewVal <<  " obs = " << hists[0]->GetBinContent(i) << endl;
  }
  ...
  TH1D *tmp_ratio_point_NoS = (TH1D *)hist_default->Clone();
  tmp_ratio_point_NoS->SetLineWidth(3.);
  tmp_ratio_point_NoS->SetLineColor(plotter.GetColor(2));
  tmp_ratio_point_NoS->Divide(hists[2]);
  
  tmp_ratio_point_NoS->Draw("histsame");
