...
void  ScaleFactorRun(TString era, TString id, TString EtaRegion, TString fit, TString SF="",int Syst = 0, TString SaveAs = "y");


void ScaleFactor(){
  
  vector<TString> SFTags = {"","_unshifted"};
  vector<TString> Eras = {"2016a","2016b","2017","2018"};

  for(auto SFtag : SFTags){
    for(auto era : Eras){
      ScaleFactorRun(era,"POGTight","BB","gaus_pol3",SFtag);
      ScaleFactorRun(era,"POGTight","EE","gaus_pol3",SFtag);
      //ScaleFactorRun(era,"TopHN","BB","gaus_pol3",i,SFtag);
      //ScaleFactorRun(era,"TopHN","EE","gaus_pol3",i,SFtag);
      ScaleFactorRun(era,"HNTightV2","BB","gaus_pol3",SFtag);
      ScaleFactorRun(era,"HNTightV2","EE","gaus_pol3",SFtag);
      if(era.Contains("16")){
	ScaleFactorRun(era,"HNL_ULID_2016","BB","gaus_pol3",SFtag);
	ScaleFactorRun(era,"HNL_ULID_2016","EE","gaus_pol3,",SFtag);
      }
      else if(era.Contains("17")){
	ScaleFactorRun(era,"HNL_ULID_2017","BB","gaus_pol3",SFtag);
	ScaleFactorRun(era,"HNL_ULID_2017","EE","gaus_pol3,",SFtag);
      }
      else{
	ScaleFactorRun(era,"HNL_ULID_2018","BB","gaus_pol3",SFtag);
	ScaleFactorRun(era,"HNL_ULID_2018","EE","gaus_pol3,",SFtag);
      }
    }
  }
  //cout << "TEST" << endl;
...
void ScaleFactorRun(TString Era, TString id, TString EtaRegion, TString fit,  TString SFTag,  int Syst = 0, TString SaveAs = "n"){ // BB, BE, EE
  ...
  TString filename= TString(std::getenv("FILE_MERGED_PATH")) + "/HNL_Lepton_ChargeFlip/"+Era+"/ScaleFactor/HNL_Lepton_ChargeFlip_SkimTreeBDT_ScaleFactor.root";
  cout << filename << endl;
  ...
  TString Outpath="/data6/Users/jalmond/2020/Plotter/HNDiLeptonWorskspace/Output/Plots//HNL_Lepton_ChargeFlip/"+samplename;
  gSystem->Exec("mkdir -p "+Outpath);
  
  TH1D* h0 = (TH1D*)f1->Get(id+"/ScaleFactor/"+EtaRegion+"_ZMass_SS");
  if(!h0) cout << id+"/ScaleFactor/"+EtaRegion+"_ZMass_SS"<< endl;;
  TH1D* h1;
  if(EtaRegion=="BB"||EtaRegion=="EE"){
    h1 = (TH1D*)f1->Get(id+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFweighted"+SFTag);
    if(!h1) cout << id+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFweighted"+SFTag << endl;
  }
  else if(EtaRegion=="BE"){
    //h1 = (TH1D*)f1->Get(id+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFSF_BWexpo_BWexpo_weighted_shifted_"+X);
    //h1 = (TH1D*)f1->Get(id+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFSF_BWexpo_gauspol3_weighted_shifted_"+X);
    //h1 = (TH1D*)f1->Get(id+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFSF_gauspol3_BWexpo_weighted_shifted_"+X);
    h1 = (TH1D*)f1->Get(id+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFSF_gauspol3_gauspol3_weighted_shifted"+SFTag );
  }
  ...
  gr->SetLineColor(15);
  gr->SetTitle("ZMass : SS2l ("+EtaRegion+")"); //);+SFTag(6,10000));
  ...
  if(fit=="BW_pol1"){
    ...
    if(EtaRegion=="BB") fitFcn->SetParameters(312,7.,-90,-20,0.9,0.,0.); //optimized - chi2/ndf : 878.528/23, SF : 153.8% XXX <--- The best chi2, 40% bkg.
    ...
    if(EtaRegion=="BB") fitFcn->SetParameters(312,7.,-90,2,-0.9); //optimized - chi2/ndf : 1076.84/25, SF : 166% XXX <--- 35% bkg.
    else if(EtaRegion=="EE") fitFcn->SetParameters(312.7,2.,-90,4.,-2.); //optimized(?) - chi2/ndf : 1903.78/25, SF : 158% XXX <--- The best Chi2, but zero bkg.
  ...
  if(fit=="gaus_pol1"){
    ...
    if(EtaRegion=="BB") fitFcn->SetParameters(480,90,3,-40,0.9,0.,0.); //optimized - chi2/ndf : 952.6/23, SF : 97.4% XXX <--- 60% bkg.
    else if(EtaRegion=="EE") fitFcn->SetParameters(480,90,3,-40,0.9,0.,0.); //optimized - chi2/ndf : 2820.46/23, SF : 107% XXX <--- 30% bkg.
    else if(EtaRegion=="BE") fitFcn->SetParameters(480,90,3,-40,0.9,0.,0.); //optimized - chi2/ndf : 8160.63/23, SF : 93.38% || chi2/ndf : same, SF : 96.35% - ??% SF at BB gives the better result..
    ...
    else if(EtaRegion=="EE") fitFcn->SetParameters(480,90,3,4,-0.9); //FIXME chi2/ndf : 7347.86/25, SF : 186.7%
  ...
  SF_t = SF_t(0,7);
  cout << "Era = " << Era << " EtaRegion = " << EtaRegion << "  ID " << id << " SF = " << SF << endl;
  ...
  gr1->SetLineColor(15);
  gr1->SetTitle("ZMass : OS_CFweighted"+SFTag +" vs SS ("+EtaRegion+") ");//+SFTag(6,10000));
  //gr1->SetMinimum(0.);
  ...
  legend->AddEntry(gr1,"OS_CFweighted"+SFTag ,"lp");
  ...
  pt2->AddText("OS2l_CF_shifted"+SFTag +" : "+OS2l_t);
  pt2->AddText("SF : "+SF_t);
  pt2->Draw();
  ...
  if(SaveAs == "y") {
    ...
    //c2->SaveAs(samplename+"/"+id+"_ScaleFactor_"+EtaRegion+"_"+".pdf");

    c1->SaveAs(Outpath+"/"+Era+"_"+id+"_SS2lFit_"+EtaRegion+"_"+SFTag+".png");
    c2->SaveAs(Outpath+"/"+Era+"_"+id+"_ScaleFactor_"+EtaRegion+"_"+SFTag+".png");
