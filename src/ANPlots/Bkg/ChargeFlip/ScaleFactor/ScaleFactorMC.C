...
void  ScaleFactorRun(TString era, TString id, TString EtaRegion, TString fit, bool MCSub=false, TString SF="",int Syst = 0, TString SaveAs = "y");


void ScaleFactor(){

  vector<bool> Subs  = {true};
  vector<TString> SFTags = {""};
  vector<TString> Eras = {"2016a","2016b","2017","2018"};
  for(auto i : Subs){
    for(auto SFtag : SFTags){
      for(auto era : Eras){
	ScaleFactorRun(era,"POGTight","BB","gaus_pol3",i,SFtag);
	ScaleFactorRun(era,"POGTight","EE","gaus_pol3",i,SFtag);
	ScaleFactorRun(era,"TopHN","BB","gaus_pol3",i,SFtag);
	ScaleFactorRun(era,"TopHN","EE","gaus_pol3",i,SFtag);
	ScaleFactorRun(era,"HNTightV2","BB","gaus_pol3",i,SFtag);
	ScaleFactorRun(era,"HNTightV2","EE","gaus_pol3",i,SFtag);
	if(era.Contains("16")){
	  ScaleFactorRun(era,"HNL_ULID_2016","BB","gaus_pol3",i,SFtag);
	  ScaleFactorRun(era,"HNL_ULID_2016","EE","gaus_pol3,",i,SFtag);
	}
	else if(era.Contains("17")){
	  ScaleFactorRun(era,"HNL_ULID_2017","BB","gaus_pol3",i,SFtag);
	  ScaleFactorRun(era,"HNL_ULID_2017","EE","gaus_pol3,",i,SFtag);
	}
	else{
	  ScaleFactorRun(era,"HNL_ULID_2018","BB","gaus_pol3",i,SFtag);
          ScaleFactorRun(era,"HNL_ULID_2018","EE","gaus_pol3,",i,SFtag);
	}
      }
    }
  }
...
void ScaleFactorRun(TString Era, TString id, TString EtaRegion, TString fit, bool MCSub, TString SFTag,  int Syst = 0, TString SaveAs = "n"){ // BB, BE, EE
  ...
  TString filename = "/data6/Users/jalmond/2020/HNDiLeptonWorskspace/InputFiles/MergedFiles/Run2UltraLegacy_v3/HNL_ChargeFlip/"+Era+"/ScaleFactor/"+SFTag+"/HNL_ChargeFlip_SkimTree_DileptonBDT_Data.root";
  TString filenameMC = "/data6/Users/jalmond/2020/HNDiLeptonWorskspace/InputFiles/MergedFiles/Run2UltraLegacy_v3/HNL_ChargeFlip/"+Era+"/ScaleFactor/"+SFTag+"/HNL_ChargeFlip_SkimTree_DileptonBDT_MC.root";

  TFile* f1 = new TFile(filename);
  TFile* f1MC = new TFile(filenameMC);
  ...
  TH1D* hmc = (TH1D*)f1MC->Get(id+RunSyst[Syst]+"/ScaleFactor/"+EtaRegion+"_ZMass_SS");
  if(MCSub)h0->Add(hmc,-1);
  ...
  if(EtaRegion=="BB"||EtaRegion=="EE"){
    ...
    //h1 = (TH1D*)f1->Get(id+RunSyst[Syst]+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFSF_BWexpo_BWexpo_weighted_shifted_"+X);
    //h1 = (TH1D*)f1->Get(id+RunSyst[Syst]+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFSF_BWexpo_gauspol3_weighted_shifted_"+X);
    //h1 = (TH1D*)f1->Get(id+RunSyst[Syst]+"/ScaleFactor/"+EtaRegion+"_ZMass_OS_CFSF_gauspol3_BWexpo_weighted_shifted_"+X);
  ...
  if(SaveAs == "y") {
    ...
    if(MCSub){
      c1->SaveAs(samplename+"/"+Era+"_"+id+"_SS2lFit_"+EtaRegion+"_"+RunSyst[Syst](6,10000)+SFTag+"_MCSub.png");
      c2->SaveAs(samplename+"/"+Era+"_"+id+"_ScaleFactor_"+EtaRegion+"_"+RunSyst[Syst](6,10000)+SFTag+"_MCSub.png");
    }
    else{
      c1->SaveAs(samplename+"/"+Era+"_"+id+"_SS2lFit_"+EtaRegion+"_"+RunSyst[Syst](6,10000)+SFTag+".png");
      c2->SaveAs(samplename+"/"+Era+"_"+id+"_ScaleFactor_"+EtaRegion+"_"+RunSyst[Syst](6,10000)+SFTag+".png");
