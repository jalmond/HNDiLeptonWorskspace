...
void PlotEnergyCFIndividualShift_PtBinned(){
...
void EnergyCFShiftProccessor(HNLPlotter Plotter,TString ID, TString Era, TString HistString, TString LabelForOutPut,vector<double> vrebin){
  ...
  for(auto ShiftVal : ShiftVals){
    ...
    hist_Prompt->GetXaxis()->SetRangeUser(-0.1,0.1);
    hist_PromptScaled->GetXaxis()->SetRangeUser(-0.1,0.1);
    ...
    TString Chi2Label3 =  "#Chi^{2}_{-0.1,0.1} = [" +trimmedString2_v3 + ","+trimmedString1_v3+"]" ;
    ...
    SaveHistogram( Plotter,{hist_CF,hist_PromptScaled,hist_Prompt}, {"El_{CF} [1]", "El_{Prompt}*Shift [2]","El_{Prompt} [3]"}, ID+"_CF_EnergyShift_"+ShiftVal+"_"+HistString, LabelForOutPut, {Chi2Label2,Chi2Label,Chi2Label3,"",MeanLabel,MeanLabel2}, HistString);
  }
