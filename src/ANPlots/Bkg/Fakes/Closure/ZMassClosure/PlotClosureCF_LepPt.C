...
void PlotClosureCF_LepPt(){
  ...
  for(auto era : Plotter.Eras()) {
    ...
    RunClosure(Plotter, "HNL_TC1_ULID_"+year, era, "May9","PBS","HNL_ChargeFlip_Closure_Z", "Separate_BinnedShift", "Closure_ElectronCF_Z" ,  {25,30,40,50,60,70,80,90,100,150,200,300,500});
    //RunClosure(Plotter, "HNL_TC1_ULID_"+year, era, "May8","CS","HNL_ChargeFlip_Closure_Z", "Separate_ConstShift","Closure_ElectronCF_Z" ,  {25,30,40,50,60,70,80,90,100,150,200,300,500});
    //RunClosure(Plotter, "HNL_TC1_ULID_"+year, era, "May8","PBS","HNL_ChargeFlip_Closure_Z", "Paired_BinnedShift", "Closure_ElectronCF_Z" ,  {25,30,40,50,60,70,80,90,100,150,200,300,500});
    //RunClosure(Plotter, "HNL_TC1_ULID_"+year, era, "May8","CS","HNL_ChargeFlip_Closure_Z", "Paired_ConstShift","Closure_ElectronCF_Z" ,  {25,30,40,50,60,70,80,90,100,150,200,300,500});

    RunClosure(Plotter, "HNL_ULID_"+year, era, "May9","PBS","HNL_ChargeFlip_Closure_Z", "Separate_BinnedShift", "Closure_ElectronCF_Z" ,  {25,30,40,50,60,70,80,90,100,150,200,300,500});
    //RunClosure(Plotter, "HNL_ULID_"+year, era, "May8","CS","HNL_ChargeFlip_Closure_Z", "Separate_ConstShift","Closure_ElectronCF_Z" ,  {25,30,40,50,60,70,80,90,100,150,200,300,500});
...
void RunClosure(HNLPlotter Plotter,TString ID, TString Era, TString  DateTag, TString Method, TString HistString, TString Indir,TString LabelForOutPut,vector<double> vrebin){
  ...
  cout << path << endl;
  TH1D *hist_mass_CF             = Plotter.ConstructHist(path,ID+"/Z/SS_Lepton1Pt", vrebin);
  
  TH1D *hist_Prompt              = Plotter.ConstructHist(path,ID+"/"+Indir+"/CFRate_InvPtEtaBin2_"+HistLab+"_Central_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  TH1D *hist_PromptUp              = Plotter.ConstructHist(path,ID+"/"+Indir+"Up/CFRate_InvPtEtaBin2_"+HistLab+"_PSigma_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  TH1D *hist_PromptDown              = Plotter.ConstructHist(path,ID+"/"+Indir+"Down/CFRate_InvPtEtaBin2_"+HistLab+"_MSigma_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  TH1D *hist_PromptStatUp              = Plotter.ConstructHist(path,ID+"/Seperate_BinnedStatUp/CFRate_InvPtEtaBin2_"+HistLab+"_Central_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  TH1D *hist_PromptStatDown              = Plotter.ConstructHist(path,ID+"/Seperate_BinnedStatDown/CFRate_InvPtEtaBin2_"+HistLab+"_Central_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt", vrebin);
  
  
  TH1D *hist_Prompt_NoShift      = Plotter.ConstructHist(path,ID+"/NoShift/CFRate_InvPtEtaBin2_NoS_Central_"+ID+"/Z/OS_Weighted_EnergyShift_Lepton1Pt" , vrebin);
  double scale = 1;
  ...
  SaveHistogram( Plotter,{hist_mass_CF,hist_Prompt,hist_Prompt_NoShift,hist_PromptUp,hist_PromptDown,hist_PromptStatUp,hist_PromptStatDown}, {"SS Observed", "OS*R_{CF} Predicted","OS*R_{CF} Predicted [no shift]","Pred Up","Pred Down"}, ID+"_CF_Closure_"+HistString+"_"+Method+"_"+Era, LabelForOutPut, {Method});
