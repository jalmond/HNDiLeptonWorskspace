...
def RunFit(channel, era, ID, HistName, HistNameMC):

  filepath= os.getenv("FILE_MERGED_PATH") + "/HNL_Lepton_ChargeFlip/"+era+"/ScaleFactor/HNL_Lepton_ChargeFlip_SkimTreeBDT_ScaleFactor.root"
  filepathMC= os.getenv("FILE_MERGED_PATH") + "/HNL_Lepton_ChargeFlip/"+era+"/ScaleFactor/HNL_Lepton_ChargeFlip_SkimTree_DileptonBDT_DYJetsToEE_MiNNLO.root"
  fileName = "FitResultsSub"
  ...
  if channel == "BE":
    histZLineShapeOS = fileTruth.Get(ID+"/ScaleFactor/"+channel+"_"+HistNameMC)
  else:
    print (ID+"/ScaleFactor/"+channel+"_"+HistName)
    histZLineShapeOS = fileTruth.Get(ID+"/ScaleFactor/"+channel+"_"+HistNameMC)
  histZLineShapeSS = fileTruth.Get(ID+"/ScaleFactor/"+channel+"_ZMass_SS")
 
  print(ID+"/ScaleFactor/"+channel+"_"+HistNameMC)
...
for Channel in Channels:
  for era in Eras:
    ...
    if era == "EB":
      RunFit(Channel, era, HNLID,"ZMass_OS_CF_PTBweighted","ZMass_OS_CFweighted")
    else:
      RunFit(Channel, era, HNLID,"ZMass_OS_CF_PTBweighted","ZMass_OS_CF_PTBweighted")
