...
void PlotEnergyCFIndividualShift_PtBinned(){
  ...
  for(auto era : Plotter.Eras()) {
    ...
    for(auto etabin :  {"EB1","EB2"}){;//,"EC"}){
      vector <TString> HistStrings = {"_PtBin1" ,"_PtBin2"};
      for(auto HistString : HistStrings ) EnergyCFShiftProccessor(Plotter, "HNL_ULID_"+year, era,etabin+HistString, "HNL_ChargeFlip_PtBinned_EnergyShift");
    }
  }
  ...
  for(auto era : Plotter.Eras()) {
    ...
    for(auto etabin :  {"EC"}){                                                                                                                                                   
      vector <TString> HistStrings = {"_PtBin1" ,"_PtBin2", "_PtBin3"};
      //for(auto HistString : HistStrings ) EnergyCFShiftProccessor(Plotter, "HNL_ULID_"+year, era,etabin+HistString, "HNL_ChargeFlip_PtBinned_EnergyShift");
    }
  }
