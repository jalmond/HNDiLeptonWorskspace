...
void PlotEnergyCFIndividualShift_PtBinnedV2(){
  ...
  for(auto era : Plotter.Eras()) {
    ...
    for(auto etabin :  {"EB1","EB2","BB"}){
      vector <TString> HistStrings = {"_Pt3Bin1" ,"_Pt3Bin2", "_Pt3Bin3" ,"_Pt3Bin4"};
      for(auto HistString : HistStrings ) EnergyCFShiftProccessor(Plotter, "HNL_ULID_"+year, era,etabin+HistString, "HNL_ChargeFlip_PtBinnedV2_EnergyShift");
    }
  }
