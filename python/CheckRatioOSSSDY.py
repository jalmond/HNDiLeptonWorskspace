...
for era in eras:
    ...
    h_dy.GetYaxis().SetRangeUser(1E-5,2)
    
    h_dy.GetYaxis().SetTitle("Ratio of SS/OS")
    ...
    for x in sig_mass_list :
        ...
        if os.path.exists("/data6/Users/jalmond/SKFlatOutput/Run2UltraLegacy_v3/HNL_SignalStudies/"+era+"/HNL_SignalStudies_"+sigDY+".root"):
            ...
            for z in range(0,h_sig.GetXaxis().GetNbins()):
                ...
                if bl in mm_os_hist:
                    tot_dy_os_mm=tot_dy_os_mm+y

                if bl in ee_os_hist:
                    tot_dy_os_ee=tot_dy_os_ee+y

                if bl in em_os_hist:
                    tot_dy_os_em=tot_dy_os_em+y
               

            print "["+sigDY+"] [MuMu] Ratio mm/ll =  "+ str(tot_dy_mm/tot_dy_os_mm)
            print "["+sigDY+"] [ElEl] Ratio ee/ll =  "+ str(tot_dy_ee/tot_dy_os_ee)
            print "["+sigDY+"] [ElMu] Ratio em/ll =  "+ str(tot_dy_em/tot_dy_os_em)

            h_dy_mm.Fill(nmass, tot_dy_mm/tot_dy_os_mm)
            h_dy_ee.Fill(nmass, tot_dy_ee/tot_dy_os_ee)
            h_dy_em.Fill(nmass, tot_dy_em/tot_dy_os_em)

            f_sig.Close()
            
            g_RSSMM.append(tot_dy_mm/tot_dy_os_mm)
            g_RSSMM_exl.append(0)
            g_RSSMM_exh.append(0)
            g_RSSMM_eyl.append(0)
            g_RSSMM_eyh.append(0)

            g_RSSEE.append(tot_dy_ee/tot_dy_os_ee)
            ...
            g_RSSEM.append(tot_dy_em/tot_dy_os_em)
    ...
    print 'Saving ==> '+outdir+'/Graph_'+era+'_Flavour_OSSS_DY.pdf'

    c1.SaveAs(outdir+'/Graph_'+era+'_FlavourOSSS_ratio_DY.pdf')
    c1.SaveAs(outdir+'/Graph_'+era+'_FlavourOSSS_ratio_DY.png')
    print ("scp " + outdir+"/Graph_"+era+"_FlavourOSSS_ratio_DY.pdf jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/OSSS/")

    os.system("scp " + outdir+"/Graph_"+era+"_FlavourOSSS_ratio_DY.pdf jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/OSSS/")
    os.system("scp " + outdir+"/Graph_"+era+"_FlavourOSSS_ratio_DY.png jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/OSSS/")
