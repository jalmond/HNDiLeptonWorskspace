...
for era in eras:
    ...
    List = GetSignalList(era,"DY") #+   GetSignalList(era,"DY") +  GetSignalList(era,"DY")

    for x in List:
        mass = x
        mass=mass.replace("DYTypeI_NLO_DF_M","")
        mass=mass.replace("DYTypeI_DF_ll_M","")
        mass=mass.replace("DYTypeI_SF_ll_M","")
        mass=mass.replace("DYTypeI_DF_M","")
...
for era in eras:
    ...
    #ROOT.gPad.Update()
    lg.AddEntry( gr_vbf_mm, 'DY MM', 'lp')
    lg.Draw()
    
    print 'Saving ==> '+outdir+'/Graph_Qratio_DY.pdf'

    c1.SaveAs(outdir+'/Graph_'+era+'_Efficiency_DY_private.pdf')
    c1.SaveAs(outdir+'/Graph_'+era+'_Efficiency_DY_private.png')

    os.system("ssh jalmond@lxplus.cern.ch 'mkdir -p /afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/Efficiency/'")    
    os.system("scp " + outdir+"/Graph_"+era+"_Efficiency_DY_private.png jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/Efficiency/")
    os.system("scp " + outdir+"/Graph_"+era+"_Efficiency_DY_private.pdf jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/Efficiency/")
