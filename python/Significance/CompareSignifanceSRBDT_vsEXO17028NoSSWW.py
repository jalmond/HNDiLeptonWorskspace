...
for channel in channels:
    ...
    for mass in masses:
        ...
        for SR in SRs:
            ...
            if mass == "1500":
                print ("DY GetEXO_17_028_Eff " + str(GetEXO_17_028_Eff(channel, Bin, mass,"DY")))
                print ("DY GetXSecUnityCoupling " + str(GetXSecUnityCoupling(dmasses[im],"DY") ))
                print ("VBF GetEXO_17_028_Eff " + str(GetEXO_17_028_Eff(channel, Bin, mass,"VBF")))
                print ("VBF GetXSecUnityCoupling " + str(GetXSecUnityCoupling(dmasses[im],"VBF") ))
    ...
    h_sig = ROOT.TH1F('h_sig_'+channel, '', 17, 0, 17)
    imass=0
    for mass in masses:
        if mass == "1400":
            continue
    ...
    for mass in masses:
        ...
        for xbin in range(1, h_SigLimitInput.GetNbinsX()+1):
            ...
            if nbkg > 0 and nsig > 0:
                ...
                if FOM == "punzi":
                    new_sig=new_sig+  nsig/ float(nsig)/math.sqrt(float(nbkgerr)*float(nbkgerr)+float(nbkg))
    ...
    print('-'*50)
    outFilePDF = outdir+'/'+channel + '_significanceComparison_BDT100-50_NoSSWW'+FOM+'.pdf'
