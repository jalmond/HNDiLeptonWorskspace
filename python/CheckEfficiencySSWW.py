...
for era in eras:
    ...
    List = GetSignalList(era,"SSWW") #+   GetSignalList(era,"SSWW") +  GetSignalList(era,"SSWW")

    for x in List:
        mass = x

        mass=mass.replace("SSWWTypeI_DF_ll_M","")
        mass=mass.replace("SSWWTypeI_SF_ll_M","")
        mass=mass.replace("SSWWTypeI_DF_M","")
        mass=mass.replace("SSWWTypeI_SF_M","")
...
for era in eras:
    ...
    h_ssww = ROOT.TH1F('h_ssww_'+era, '', 3000, 0, 3000) 
    ...
    h_ssww.GetYaxis().SetRangeUser(1E-5,2)
    ...
    for x in sig_mass_list :
        ...
        sigSSWW =  "SSWWTypeI_SF_ll_M"+str(x)
        ...
        if os.path.exists("/data6/Users/jalmond/SKFlatOutput/Run2UltraLegacy_v3/HNL_SignalStudies/"+era+"/HNL_SignalStudies_"+sigSSWW+".root"):
            ...
            h_sig_ee = f_sig.Get("EE_ChannelSignalsNewSel_SignalStudy/FillEventCutflow_SR/SR_Summary")

            eff_EE=0

            if h_sig_ee:
                eff_EE =  h_sig_ee.GetBinContent(6) / h_sig_ee.GetBinContent(1)


            h_ssww_ee.Fill(nmass,eff_EE)

            f_sig.Close()

            g_RSSEE.append(eff_EE)
    ...
    #gr_ssww_tot.SetFillColor(ROOT.kGreen-2)
    gr_ssww_mm.SetFillColor(ROOT.kGreen-2)
    gr_ssww_ee.SetFillColor(ROOT.kRed)
    #gr_ssww_em.SetFillColor(ROOT.kCyan)
    ...
    gr_ssww_mm.SetFillStyle(3010)
    gr_ssww_mm.Draw('CpsameE3')
    
    gr_ssww_ee.SetFillStyle(3010)
    gr_ssww_ee.Draw('CpsameE3')
    ...
    c1.SaveAs(outdir+'/Graph_'+era+'_Efficiency_SSWW_private.pdf')
    c1.SaveAs(outdir+'/Graph_'+era+'_Efficiency_SSWW_private.png')

    os.system("ssh jalmond@lxplus.cern.ch 'mkdir -p /afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/Efficiency/'")    
    os.system("scp " + outdir+"/Graph_"+era+"_Efficiency_SSWW_private.png jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/Efficiency/")
    os.system("scp " + outdir+"/Graph_"+era+"_Efficiency_SSWW_private.pdf jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/Efficiency/")
