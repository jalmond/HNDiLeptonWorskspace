...
if args.Category==0:
  #### Define Samples
  if args.Year>0:
    exec('m.SampleGroups = [SampleGroup_Top_%s, SampleGroup_WZ_powheg_%s,  SampleGroup_ZZ_powheg_%s,SampleGroup_NonPrompt_Electron_%s, SampleGroup_VVV_%s , SampleGroup_DY_%s , SampleGroup_CF_%s,SampleGroup_Others_%s,SampleGroup_ZG_%s, SampleGroup_WG_%s ]'%(args.Year,args.Year, args.Year,args.Year,args.Year,args.Year,args.Year,args.Year,args.Year,args.Year))
  else:
    m.SampleGroups = [
  ...
  #### Define reiongs
  m.RegionsToDraw = [
    Region('FillEventCutflow' , 'Electron', UnblindData=True, Logy=1, TLatexAlias='#splitline{ee}{CutFlow}',CutFlowCaption="Single Electron Trigger On Z shell, OS dilepton pair"),
...
m.VariablesToDraw = [
  ...
  Variable('OptID_Electron_Inclusive', '', ''),
  Variable('OptID_Electron_v1', '', ''),
  Variable('OptID_Electron_v2', '', ''),
