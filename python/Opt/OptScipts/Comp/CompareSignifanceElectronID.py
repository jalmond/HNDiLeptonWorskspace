...
def GetSignificance(histname,mass,f_Bkg):
    ...
    #for xbin in range(1,8):
        ...
        if nbkg > 0:
            ...
            #print "Bin " + str(xbin) + " nsig = " + str(nsig) + " bkg = " + str(nbkg) + " sig = " + float(nsig)/math.sqrt(float(bkgerr)*float(bkgerr)+float(nbkg))  #str(float(nsig)/float(math.sqrt(nbkg)))
...
for channel in channels:
    ...
    for k, o in getall(f_Bkg):
        print o.ClassName(), k
        if not "ElectronChargeSplitSR" in k:
            continue
        if not "MVABB" in k:
            continue
    ...
    for mass in masses:
        ...
        c1.SaveAs(outdir+'/'+channel + '_OptElectron_'+str(mass)+'.pdf')
        c1.SaveAs(outdir+'/'+channel + '_OptElectron_'+str(mass)+'.png')
        ...
        os.system("scp " + outdir+"/"+channel + "_OptElectron_"+str(mass)+".png jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/Opt/")
        os.system("scp " + outdir+"/"+channel + "_OptElectron_"+str(mass)+".pdf jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/Opt/")
        print ("scp " + outdir+"/"+channel + "_OptElectron_"+str(mass)+".pdf jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/Opt/")
