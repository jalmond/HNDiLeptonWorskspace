...
def GetSignificance(nIt,histname,h_LimitInput,h_SigLimitInput,A,B,C):
...
for channel in channels:
    ...
    for x in SRNames:
        for y  in EtaNames:
            ...
            if "SR1" in x:
                HistNames.append("ISOOpt/"+x+args.MVA+"__"+y+"Pt_Iso_HNLOpt_UL_HNOpt")
            else:
                HistNames.append("ISOOpt/"+x+args.MVA+"__"+y+"Pt_Iso_HNLOpt_UL_HNOpt")
    ...
    for mass in masses:
        ...
        for _A in range(0, NhistnamesX):
            for _B in range(0, histnamesY):
                ...
                value_A = 0.01 + float(_A)*0.002
                value_B = 0.01 + float(_B)*0.002
                value_C = 0
                ...
                for X in range(0,len(SigHists)):
                        ...
                        hnsigni = hnsigni + GetHNSignificance(nIt,HNHistNames[X],hnhistBkg,hnhistSig)
                ...
                for X in range(0,len(SigHists)):
                    ...
                    sig_h= GetSignificance(nIt,HistNames[X],histBkg,histSig,value_A,value_B,value_C)
        ...
        filename= channel + '_OptIsoElectron_'+args.EtaRegion+'_3D_'+str(mass)+"_"+str(args.NProc)+ "_"+str(args.MVA)+"__"+args.BinType
        if args.Data:
            filename= channel + '_OptIsoElectron_'+args.EtaRegion+'_3D_'+str(mass)+"_"+str(args.NProc)+ "_"+str(args.MVA)+"__"+args.BinType +"__DATA"
