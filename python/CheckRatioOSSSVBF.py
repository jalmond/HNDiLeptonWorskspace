...
for era in eras:
    ...
    outdir = PLOT_PATH+'/CheckRatioFlavOSSS/'+era+'/'
    os.system("mkdir -p " + PLOT_PATH+'/CheckRatioFlavOSSS/')
    os.system("mkdir -p " + PLOT_PATH+'/CheckRatioFlavOSSS/'+era)
    ...
    h_vbf.GetYaxis().SetTitle("Ratio of SS/OS")
    ...
    for x in sig_mass_list :
        ...
        if os.path.exists("/data6/Users/jalmond/SKFlatOutput/Run2UltraLegacy_v3/HNL_SignalStudies/"+era+"/HNL_SignalStudies_"+sigVBF+".root"):
            ...
            for z in range(0,h_sig.GetXaxis().GetNbins()):
                ...
                if bl in mm_os_hist:
                    tot_vbf_os_mm=tot_vbf_os_mm+y

                if bl in ee_os_hist:
                    tot_vbf_os_ee=tot_vbf_os_ee+y

                if bl in em_os_hist:
                    tot_vbf_os_em=tot_vbf_os_em+y
               

            print "["+sigVBF+"] [MuMu] Ratio mm/ll =  "+ str(tot_vbf_mm/tot_vbf_os_mm)
            print "["+sigVBF+"] [ElEl] Ratio ee/ll =  "+ str(tot_vbf_ee/tot_vbf_os_ee)
            print "["+sigVBF+"] [ElMu] Ratio em/ll =  "+ str(tot_vbf_em/tot_vbf_os_em)

            h_vbf_mm.Fill(nmass, tot_vbf_mm/tot_vbf_os_mm)
            h_vbf_ee.Fill(nmass, tot_vbf_ee/tot_vbf_os_ee)
            h_vbf_em.Fill(nmass, tot_vbf_em/tot_vbf_os_em)

            f_sig.Close()
            
            g_RSSMM.append(tot_vbf_mm/tot_vbf_os_mm)
            g_RSSMM_exl.append(0)
            g_RSSMM_exh.append(0)
            g_RSSMM_eyl.append(0)
            g_RSSMM_eyh.append(0)

            g_RSSEE.append(tot_vbf_ee/tot_vbf_os_ee)
            ...
            g_RSSEM.append(tot_vbf_em/tot_vbf_os_em)
    ...
    print 'Saving ==> '+outdir+'/Graph_'+era+'_Flavour_OSSS_VBF.pdf'

    c1.SaveAs(outdir+'/Graph_'+era+'_FlavourOSSS_ratio_VBF.pdf')
    c1.SaveAs(outdir+'/Graph_'+era+'_FlavourOSSS_ratio_VBF.png')
    print ("scp " + outdir+"/Graph_"+era+"_FlavourOSSS_ratio_VBF.pdf jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/OSSS/")

    os.system("scp " + outdir+"/Graph_"+era+"_FlavourOSSS_ratio_VBF.pdf jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/OSSS/")
    os.system("scp " + outdir+"/Graph_"+era+"_FlavourOSSS_ratio_VBF.png jalmond@lxplus.cern.ch:/afs/cern.ch/user/j/jalmond/www/SNU/WebPlots/HNL/SignalStudies/OSSS/")
