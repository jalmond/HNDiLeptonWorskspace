...
def GetSignificance(out,h_sig, h_bkg):
    ...
    for x in range(0, len(BkgBins)):
        ...
        Print(out, "Bin " + str(h_sig.GetXaxis().GetBinLabel(x+1)) + " s/sqrt(B+1) = " + str(SigBins[x]/ math.sqrt(Bkg +1)) + " sig = " + str(SigBins[x]) + " bkg = " + str(Bkg))
...
for channel in channels:
    ...
    for mass in masses:
        ...
        DirName = "LimitInput"
        HistName="/HNLOpt_UL_HNTightV2/LimitBins/ElectronSR"
        if mass == "100":
            DirName = "LimitInputBDT"
            HistName="/HNLOpt_UL_HNTightV2/100/LimitBins/ElectronSR"
        ...
        for x in HistNames:
            ...
            if not _hist in sample_dict:
                ...
                #/HNLOpt_ULHNTightPFIsoLoose/500/LimitBins/ElectronSR x
