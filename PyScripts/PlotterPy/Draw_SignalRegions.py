...
from CRPlotter import SampleGroup, Variable, Region, Systematic,SignalInfo
...
SignalInfoToDraw = SignalInfo(100)
SignalInfoToDraw.useOfficial = True
SignalInfoToDraw.Color = ROOT.kBlack
SignalInfoToDraw.Style = 5
SignalInfoToDraw.xsec = 1.
SignalInfoToDraw.kfactor = 1
...
if args.Category==0:
  ...
  m.SignalsToDraw = [SignalInfoToDraw]
  ...
  m.RegionsToDraw = [
    ...
    Region('RegionPlots_InclusiveSR1', 'MuMu', PNs[0],  UnblindData=False, Logy=0, TLatexAlias='#splitline{#mu#mu}{DY->ll CR (M>50)}', CutFlowCaption='Number of Events in Zmass Window (SingleMuon Trigger). Truth matching applied' ),
...
m.VariablesToDraw = [
  ...
  Variable('AK8Jet_Mass_l1J', 'M_{l1J} GeV',''),
